...
table 8063 "Vendor Subscription Contract"
{
        ...
        field(3; "No."; Code[20])
        {
            ...
            begin
                if "No." <> xRec."No." then begin
                    GetServiceContractSetup();
                    NoSeries.TestManual(ServiceContractSetup."Vend. Sub. Contract Nos.");
                    "No. Series" := '';
                end;
            end;
        }
    ...
    begin
        ...
        VendorContractLine.SetRange("Subscription Contract No.", "No.");
    ...
    var
        ...
        ModifyPayToVendorAddressNotificationDescriptionTxt: Label 'Warn if the Pay-to address on Vendor Subscription Contract is different from the Vendor''s existing address.';
    ...
    begin
        ...
        if not IsHandled then
            if "No." = '' then begin
                TestNoSeries();
                "No. Series" := ServiceContractSetup."Vend. Sub. Contract Nos.";
    ...
    internal procedure AssistEdit(OldVendContract: Record "Vendor Subscription Contract"): Boolean
    var
        IsHandled: Boolean;
    begin
        IsHandled := false;
        ...
        if NoSeries.LookupRelatedNoSeries(ServiceContractSetup."Vend. Sub. Contract Nos.", OldVendContract."No. Series", "No. Series") then begin
            "No." := NoSeries.GetNextNo("No. Series");
            exit(true);
        end;
    end;
    ...
    begin
        GetServiceContractSetup();
        ServiceContractSetup.TestField("Vend. Sub. Contract Nos.");
    end;
    ...
    local procedure VendorContractLinesExists(var VendorContractLine: Record "Vend. Sub. Contract Line"): Boolean
    begin
        VendorContractLine.Reset();
        VendorContractLine.SetRange("Subscription Contract No.", Rec."No.");
    ...
    begin
        ...
        VendorContractDeferral.SetRange("Subscription Contract No.", Rec."No.");
    ...
    internal procedure OnAfterValidateBuyFromVendorNo(var VendorContract: Record "Vendor Subscription Contract"; var xVendorContract: Record "Vendor Subscription Contract")
    begin
    ...
    begin
        ...
        ModifyVendorAddressNotification.AddAction(NotificationLbl, Codeunit::"Sub. Contract Notifications", NotificationFunctionTok);
        ModifyVendorAddressNotification.AddAction(
          DontShowAgainActionLbl, Codeunit::"Sub. Contract Notifications", 'VendorContractHideNotificationForCurrentUser');
    ...
    local procedure ValidatePurchaserOnVendorContract(VendorContract2: Record "Vendor Subscription Contract")
    begin
    ...
    internal procedure CreateVendorContractLinesFromServiceCommitments(var ServiceCommitment: Record "Subscription Line" temporary)
    var
    ...
    begin
        if NotReleasedVendorContractDeferralsExists() then
            if VendorContractLinesExists(VendorContractLine) then begin
                VendorContractLine.SetFilter("Subscription Line Entry No.", '<>0');
                if VendorContractLine.FindSet() then
                    repeat
                        if ServiceCommitment.Get(VendorContractLine."Subscription Line Entry No.") then begin
                            VendorContractDeferral.SetRange("Subscription Contract Line No.", VendorContractLine."Line No.");
                            VendorContractDeferral.SetRange(Released, false);
