...
page 12216 "Periodic VAT Settl. List"
{
    ...
    actions
    {
        area(Processing)
        {
            action(SplitPeriodicEntry)
            {
                ApplicationArea = All;
                Caption = 'Split Periodic Entry';

                trigger OnAction()
                var
                    PeriodicVATSettlementEntry: Record "Periodic VAT Settlement Entry";
                    VATSetup: Record "VAT Setup";
                    PeriodicVATSplit: Page "Periodic VAT Split";
                ...
                begin
                    ...
                    Commit();
                    PeriodicVATSettlementEntry.SetRange("VAT Period", Rec."VAT Period");
                    PeriodicVATSplit.SetTableView(PeriodicVATSettlementEntry);
                    PeriodicVATSplit.Run();
                end;
            }
        }
        area(Promoted)
        {
            actionref(SplitPeriodicEntry_Promoted; SplitPeriodicEntry) { }
        }
