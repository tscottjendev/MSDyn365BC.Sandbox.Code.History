...
#pragma warning restore AS0074
    ...
    fields
    {
        ...
        field(12139; "Primary Key"; Text[250])
        {
            Caption = 'Primary Key';
            DataClassification = SystemMetadata;
        }
...
#pragma warning disable AS0072
    ...
    begin
        ...
        "Include in VAT Transac. Rep." := SalesLine."Include in VAT Transac. Rep.";
        "Refers to Period" := SalesLine."Refers to Period";
        "Contract No." := SalesLine."Blanket Order No.";
        ...
        if VATPostingSetup.IsEUService(SalesLine."VAT Bus. Posting Group", SalesLine."VAT Prod. Posting Group") then begin
            "Service Tariff No." := SalesLine."Service Tariff No.";
    ...
    begin
        ...
        "Deductible %" := PurchLine."Deductible %";
        "VAT Identifier" := PurchLine."VAT Identifier";
        if VATPostingSetup.IsEUService(PurchLine."VAT Bus. Posting Group", PurchLine."VAT Prod. Posting Group") then begin
            "Service Tariff No." := PurchLine."Service Tariff No.";
            "Transport Method" := PurchLine."Transport Method";
        end;
        "Related Entry No." := PurchLine."Related Entry No.";
        "Include in VAT Transac. Rep." := PurchLine."Include in VAT Transac. Rep.";
        "Refers to Period" := PurchLine."Refers to Period";
        "Contract No." := PurchLine."Blanket Order No.";
        ...
        if Type = Type::"Fixed Asset" then begin
            ...
            "No. of Fixed Asset Cards" := PurchLine."No. of Fixed Asset Cards";
        end;
    ...
    begin
        ...
        "VAT Identifier" := ServiceLine."VAT Identifier";
        "Include in VAT Transac. Rep." := ServiceLine."Include in VAT Transac. Rep.";
        "Refers to Period" := ServiceLine."Refers to Period";
        "Contract No." := ServiceLine."Contract No.";
        ...
        "VAT %" := ServiceLine."VAT %";
        if VATPostingSetup.IsEUService(ServiceLine."VAT Bus. Posting Group", ServiceLine."VAT Prod. Posting Group") then begin
            "Service Tariff No." := ServiceLine."Service Tariff No.";
    ...
    begin
        InvoicePostBuffer.FillInvPostingBufferPrimaryKey();
    ...
    procedure FillInvPostingBufferPrimaryKey()
    var
        TypeValue: Integer;
        RefersToPeriod: Integer;
    begin
        TypeValue := Type.AsInteger();
        RefersToPeriod := "Refers to Period";
        "Primary Key" :=
          Format(TypeValue) + "G/L Account" + "Gen. Bus. Posting Group" + "Gen. Prod. Posting Group" +
          "VAT Bus. Posting Group" + "VAT Prod. Posting Group" + "Tax Area Code" + "Tax Group Code" +
          Format("Tax Liable") + Format("Use Tax") + Format("Dimension Set ID") + "Job No." +
          Format("Fixed Asset Line No.") + "Deferral Code" + "Service Tariff No." + Format("Related Entry No.") +
          Format("Include in VAT Transac. Rep.") + Format(RefersToPeriod) + "Contract No.";

        OnAfterFillInvPostingBufferPrimaryKey(Rec);
    end;
