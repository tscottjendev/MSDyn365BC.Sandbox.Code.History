        ...
        area(navigation)
        {
            group("O&CR")
            {
                ...
                action(ImportPayments)
                {
                    ...
                    Image = Import;
                    ToolTip = 'Import a payment file from Bankenes BetalingsSentral (BBS) for optical character recognition (OCR). Note that OCR payment do not use a balance account in the Bal. Account No. field on the cash receipt journal line.';
                ...
                {
                    ApplicationArea = Basic, Suite;
                    Caption = 'OCR control report';
                    Image = "Report";
                    ToolTip = 'View the imported payment transactions in the cash receipt journal. You can also view potential warnings and reconcile conflicts before the payments are posted to the journal.';

                    trigger OnAction()
                    var
                        OCRControlReport: Report "OCR Journal - Test";
                    begin
                ...
                action("Show warning")
                {
                    ...
                    Image = Info;
                    RunObject = Page "OCR Return Info";
                    RunPageLink = "Journal Template Name" = field("Journal Template Name"),
                                  "Journal Batch Name" = field("Journal Batch Name"),
                                  "Line No." = field("Line No.");
                    ToolTip = 'View if there is a warning for the optical character recognition (OCR) return. If selected, the warning text will be shown.';
                }
                action("Warning from/to")
                {
                    ApplicationArea = Basic, Suite;
                    Caption = 'Warning from/to';
                    Image = UpdateDescription;
                    ShortCutKey = 'Ctrl+F9';

                    trigger OnAction()
                    begin
                        Rec.Validate(Warning, not Rec.Warning);
                        Rec.Modify();
                        CurrPage.Update();
                    end;
                }
