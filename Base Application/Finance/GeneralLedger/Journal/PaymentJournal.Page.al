...
page 256 "Payment Journal"
{
    ...
    layout
    {
        area(content)
        {
            ...
            repeater(Control1)
            {
                ...
                field("Currency Code"; Rec."Currency Code")
                {
                    ...
                    trigger OnAssistEdit()
                    begin
                        if Rec."Source Code" = 'DTA' then
                            if not ((Rec."Applies-to Doc. No." <> '') or
                               ((Rec."Currency Code" <> '') and
                               (Rec."Currency Code" = CopyStr(Rec.Description, (StrLen(Rec.Description) - 3), 3))))
                            then
                                Error(Text90600);
                        ...
                        if DtaSetup.ReadPermission and (Rec."Currency Factor" <> xRec."Currency Factor") then begin
                            DtaMgt.ModifyExRate(Rec);
                            CurrPage.Update();
                        end;
                    end;
                }
                ...
                {
                    ApplicationArea = Basic, Suite;
                    ToolTip = 'Specifies the reference number transferred from the deposit slip and stored for ESR invoices. It will be used again later if it is transferred to the bank in the DTA file as part of the payment order.';
                }
        ...
        area(factboxes)
        {
            ...
            part(Control1906888707; "Payment Journal FactBox")
            {
                ApplicationArea = Basic, Suite;
                SubPageLink = "Journal Template Name" = field("Journal Template Name"),
                              "Journal Batch Name" = field("Journal Batch Name"),
        ...
        area(navigation)
        {
            group("&DTA")
            {
                ...
                action("Show &Open Invoices")
                {
                    ...
                    Image = Invoice;
                    ToolTip = 'View invoices that are not fully paid.';
                ...
                action("Generate &DTA File")
                {
                    ...
                    Image = Export;
                    ToolTip = 'Create a file to prepare to submit DatenTrÃ”Ã‡Ã—gerAustausch (DTA) payments to your bank for payment.';
                ...
                action("DTA &Payment Order")
                {
                    ...
                    Image = "Report";
                    ToolTip = 'View the summarized suggested payments from a log for each currency code. This payment order is sent to the bank so that the DTA file is released for payment.';
                ...
                action("Generate EZAG File")
                {
                    ...
                    Image = Export;
                    ToolTip = 'Create a file to prepare to submit Elektronischer Zahlungsauftrag (EZAG) (EZAG) payments to your bank for payment.';
                ...
                action("E&ZAG Payment Order")
                {
                    ...
                    Image = "Report";
                    ToolTip = 'View report groups for all suggested payments from a log for each currency code. This payment order is sent to the bank so that the DTA file for payment is released.';
                ...
                action("Modify Document &No.")
                {
                    ApplicationArea = Basic, Suite;
                    Caption = 'Modify Document &No.';
                    Image = DocumentEdit;
                    ToolTip = 'Change the number of a document in the journal.';
                ...
                action("Update &Balance Account Lines")
                {
                    ApplicationArea = Basic, Suite;
            ...
            group("A&ccount")
            {
                ...
                action("&Balance")
                {
                    ...
                    Caption = '&Balance';
                    Image = Balance;
                    RunObject = Page "G/L Acc. Provisional Balance";
                    RunPageLink = "Journal Template Name" = field("Journal Template Name"),
                                  "Journal Batch Name" = field("Journal Batch Name"),
                                  "Line No." = field("Line No.");
                    RunPageOnRec = true;
                    ShortCutKey = 'F7';
                    ToolTip = 'View the temporary general ledger balance. It is the balance of the final posted ledger entries and the balance of the postings entered in the current journal.';
                }
            }
            group("&Payments")
            {
...
#pragma warning restore AA0219
        ...
        area(Promoted)
        {
            ...
            group(Category_Report)
            {
                ...
                actionref("Print Payment Journal_Promoted"; "Print Payment Journal")
                {
                }
                actionref("&Print Payment Ad&vice_Promoted"; "&Print Payment Ad&vice")
                {
                }
                actionref("DTA &Payment Order_Promoted"; "DTA &Payment Order")
                {
                }
                actionref("E&ZAG Payment Order_Promoted"; "E&ZAG Payment Order")
                {
                }
            }
            group(Category_Category12)
            {
                ...
                actionref("Show &Open Invoices_Promoted"; "Show &Open Invoices")
                {
                }
                actionref("Generate &DTA File_Promoted"; "Generate &DTA File")
                {
                }
                actionref("Generate EZAG File_Promoted"; "Generate EZAG File")
                {
                }
                actionref("Start &Yellownet_Promoted"; "Start &Yellownet")
                {
                }
                actionref("Modify Document &No._Promoted"; "Modify Document &No.")
                {
                }
                actionref("Update &Balance Account Lines_Promoted"; "Update &Balance Account Lines")
                {
                }
            }
        }
    ...
    local procedure RunReportWithCurrentRec(ReportID: Integer)
    var
