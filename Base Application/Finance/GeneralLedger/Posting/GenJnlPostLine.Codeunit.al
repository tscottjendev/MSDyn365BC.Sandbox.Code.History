...
codeunit 12 "Gen. Jnl.-Post Line"
{
                    ...
                    begin
                        ...
                        case GenJnlLine."VAT Calculation Type" of
                            ...
                            GenJnlLine."VAT Calculation Type"::"Sales Tax":
                                begin
                                    if GenJnlLine."Tax Area Code" = '' then
                                        Clear(TaxArea)
                                    else
                                        TaxArea.Get(GenJnlLine."Tax Area Code");
                                            ...
                                            ExpenseAmount :=
                                              Round(
                                                SalesTaxCalculate.CalculateExpenseTax(
                                                  GenJnlLine."Tax Area Code", GenJnlLine."Tax Group Code", GenJnlLine."Tax Liable",
                                                  GenJnlLine."Posting Date", GLEntry.Amount, GenJnlLine.Quantity, 0));
                                        GLEntry."VAT Amount" := GenJnlLine."Amount (LCY)" - GLEntry.Amount - ExpenseAmount;
                                        GLEntry.Amount := GLEntry.Amount + ExpenseAmount;
                                        if ExpenseAmount <> 0 then begin
                                            AddCurrExpense := CalcLCYToAddCurr(ExpenseAmount);
                                            GenJnlLine."Source Curr. VAT Base Amount" += AddCurrExpense;
                                            GenJnlLine."Source Curr. VAT Amount" := GenJnlLine."Source Currency Amount" - GenJnlLine."Source Curr. VAT Base Amount";
                                        end;
                                    end;
                                    GLEntry."Additional-Currency Amount" := GenJnlLine."Source Curr. VAT Base Amount";
                                    if GenJnlLine."Source Currency Code" = AddCurrencyCode then
                ...
                case GenJnlLine."VAT Posting" of
                        ...
                        begin
                            ...
                            while SalesTaxCalculate.GetSalesTaxLine(TaxDetail2, VATAmount, VATBase) do begin
                                ...
                                if TaxDetailFound then begin
                                    GenJnlLine."Tax Jurisdiction Code" := TaxDetail."Tax Jurisdiction Code";
                    ...
                    GenJnlLine."VAT Posting"::"Manual VAT Entry":
                        if GenJnlLine."Gen. Posting Type" = GenJnlLine."Gen. Posting Type"::Settlement then begin
                            ...
                            TaxDetail."Tax Jurisdiction Code" := GenJnlLine."Tax Area Code";
                            ...
                            InsertVAT(
                                GenJnlLine, VATPostingSetup,
                                GenJnlLine."VAT Amount (LCY)", GenJnlLine."VAT Amount (LCY)", GenJnlLine."VAT Base Amount (LCY)", GenJnlLine."Source Currency Code",
                                GenJnlLine."Source Curr. VAT Amount", GenJnlLine."Source Curr. VAT Amount", GenJnlLine."Source Curr. VAT Base Amount");
                            InsertSummarizedVAT(GenJnlLine);
                        end;
                end;
        end;
    ...
    begin
        ...
        VATEntry."Tax Exemption No." := GenJnlLine."Tax Exemption No.";
        VATEntry."Tax Jurisdiction Code" := GenJnlLine."Tax Jurisdiction Code";
        VATEntry."STE Transaction ID" := GenJnlLine."STE Transaction ID";
        VATEntry."GST/HST" := GenJnlLine."GST/HST";
                    ...
                    VATDifferenceLCY :=
                      Round(
                        CurrExchRate.ExchangeAmtFCYToLCY(
                          GenJnlLine."Posting Date", SrcCurrCode, GenJnlLine."VAT Difference",
                          CurrExchRate.ExchangeRate(GenJnlLine."Posting Date", SrcCurrCode)));
        end else
            if GenJnlLine."VAT Difference" = 0 then
                VATDifferenceLCY := 0
            else
        ...
        if GenJnlLine."VAT Calculation Type" = GenJnlLine."VAT Calculation Type"::"Sales Tax" then begin
            if SkipFindTaxDetail(TaxDetail, GenJnlLine) then
                exit;
                ...
                GenJnlLine."Gen. Posting Type"::Purchase:
                    case VATPostingSetup."VAT Calculation Type" of
                        ...
                        VATPostingSetup."VAT Calculation Type"::"Sales Tax":
                            if GenJnlLine."VAT Posting" = GenJnlLine."VAT Posting"::"Automatic VAT Entry" then begin
                                if GenJnlLine."Use Tax" then begin
                                ...
                                InitGLEntryVAT(
                                  GenJnlLine, GenJnlLine."Account No.", '', VATPostingParameters."Full VAT Amount", VATPostingParameters."Full VAT Amount ACY", true);
                    end;
                GenJnlLine."Gen. Posting Type"::Sale:
                    case VATPostingSetup."VAT Calculation Type" of
                        VATPostingSetup."VAT Calculation Type"::"Normal VAT",
                      VATPostingSetup."VAT Calculation Type"::"Full VAT":
                        ...
                        VATPostingSetup."VAT Calculation Type"::"Sales Tax":
                            if GenJnlLine."VAT Posting" = GenJnlLine."VAT Posting"::"Automatic VAT Entry" then
                                ...
                                // Manual VAT Entry
                                InitGLEntryVAT(GenJnlLine, GenJnlLine."Account No.", '', VATPostingParameters."Full VAT Amount", VATPostingParameters."Full VAT Amount ACY", true);
                    end;
            end;
    end;
    ...
    begin
        ...
        if ((GenJnlLine.Amount <= 0) and (GenJnlLine."Bank Payment Type" = GenJnlLine."Bank Payment Type"::"Computer Check") and GenJnlLine."Check Printed") or
           ((GenJnlLine."Bank Payment Type" = GenJnlLine."Bank Payment Type"::"Electronic Payment") and GenJnlLine."Check Transmitted") or
           ((GenJnlLine."Bank Payment Type" = GenJnlLine."Bank Payment Type"::"Electronic Payment") and GenJnlLine."Check Exported") or
           ((GenJnlLine."Bank Payment Type" = GenJnlLine."Bank Payment Type"::"Electronic Payment-IAT") and GenJnlLine."Check Transmitted") or
           ((GenJnlLine."Bank Payment Type" = GenJnlLine."Bank Payment Type"::"Electronic Payment-IAT") and GenJnlLine."Check Exported") or
        ...
        then begin
            ...
            case GenJnlLine."Bank Payment Type" of
                ...
                GenJnlLine."Bank Payment Type"::"Electronic Payment", GenJnlLine."Bank Payment Type"::"Electronic Payment-IAT":
                    begin
                        if GenJnlLine."Bank Payment Type" = GenJnlLine."Bank Payment Type"::"Electronic Payment-IAT" then
                            GenJnlLine.TestField(GenJnlLine."Check Exported")
                        else
                            if BankAcc."Export Format" = BankAcc."Export Format"::Other then
                                // AMC
                                GenJnlLine.TestField(GenJnlLine."Check Transmitted", true)
                            else
                                GenJnlLine.TestField(GenJnlLine."Check Exported");
                        CheckLedgEntry.LockTable();
                        CheckLedgEntry.Reset();
                        CheckLedgEntry.SetCurrentKey("Bank Account No.", "Entry Status", "Check No.");
                        CheckLedgEntry.SetRange("Bank Account No.", GenJnlLine."Account No.");
                        CheckLedgEntry.SetFilter("Entry Status", '%1|%2|%3',
                          CheckLedgEntry."Entry Status"::Transmitted,
                          CheckLedgEntry."Entry Status"::Printed,
                          CheckLedgEntry."Entry Status"::Exported);
                        CheckLedgEntry.SetRange("Check No.", GenJnlLine."Document No.");
        ...
        if VATEntry2.FindSet() then begin
            ...
            repeat
                ...
                if VATPart > 0 then begin
                    ...
                    PostUnrealVATEntry(
                      GenJnlLine, VATEntry2, VATAmount, VATBase, VATAmountAddCurr, VATBaseAddCurr, GLEntryNo,
    ...
    begin
        ...
        repeat
            ...
            PostApply(
              GenJnlLine, DtldCVLedgEntryBuf, OldCVLedgEntryBuf, NewCVLedgEntryBuf, NewCVLedgEntryBuf2,
              true, AllApplied, AppliedAmount, PmtTolAmtToBeApplied, VATRealizedGainLossLCY);
            ...
            repeat
                ...
                if VATPart > 0 then begin
                    ...
                    then begin
                        ...
                        GLEntryNo :=
                          InitGLEntryVATCopy(
                            GenJnlLine, PurchReverseAccount, PurchReverseUnrealAccount,
                            VATAmount, VATAmountAddCurr, VATEntry2);
                    end else begin
                        IsHandled := false;
    ...
    begin
        ...
        VATEntry."Realized Amount" := RealizedVATAmount;
        VATEntry."Realized Base" := RealizedVATBase;
        VATEntry."Add.-Curr. Realized Amount" := RealizedVATAmountAddCurr;
        VATEntry."Add.-Curr. Realized Base" := RealizedVATBaseAddCurr;
        VATEntry."G/L Acc. No." := '';
        ...
        VATEntry2."Remaining Unrealized Amount" :=
          VATEntry2."Remaining Unrealized Amount" - VATAmount;
        VATEntry2."Remaining Unrealized Base" :=
          VATEntry2."Remaining Unrealized Base" - VATBase;
        ...
        VATEntry2."Realized Amount" -= RealizedVATAmount;
        VATEntry2."Realized Base" -= RealizedVATBase;
    ...
    var
        ...
        AmountAddCurrCash: Decimal;
        GLEntryNoFromVAT: Integer;
    begin
        OnBeforePostUnrealVATByUnapply(GenJnlLine, VATPostingSetup, VATEntry, NewVATEntry);

        if (VATEntry."Realized Amount" <> 0) and (GenJnlLine."Source Currency Code" = '') then
            Error(CannotUnapplyEmptySrcCurrErr, VATEntry.FieldCaption("Realized Amount"), GenJnlLine.FieldCaption("Source Currency Code"));
        ...
        if VATEntry."Realized Amount" <> 0 then
            CreateGLEntryFromVATEntry(
              GenJnlLine, GetRealizedVATGainLossGLAccNo(VATEntry, GenJnlLine."Source Currency Code"),
              VATEntry."Realized Amount", AmountAddCurrCash, VATEntry);

        VATEntry2.Get(VATEntry."Unrealized VAT Entry No.");
        VATEntry2."Remaining Unrealized Amount" := VATEntry2."Remaining Unrealized Amount" - NewVATEntry.Amount - VATEntry."Realized Amount";
        VATEntry2."Remaining Unrealized Base" := VATEntry2."Remaining Unrealized Base" - NewVATEntry.Base - VATEntry."Realized Base";
        VATEntry2."Add.-Curr. Rem. Unreal. Amount" :=
          VATEntry2."Add.-Curr. Rem. Unreal. Amount" - NewVATEntry."Additional-Currency Amount" - NewVATEntry."Add.-Curr. Realized Amount";
        VATEntry2."Add.-Curr. Rem. Unreal. Base" :=
          VATEntry2."Add.-Curr. Rem. Unreal. Base" - NewVATEntry."Additional-Currency Base" - NewVATEntry."Add.-Curr. Realized Base";
        VATEntry2."Realized Amount" -= NewVATEntry."Realized Amount";
        VATEntry2."Realized Base" -= NewVATEntry."Realized Base";
        VATEntry2."Add.-Curr. Realized Amount" -= NewVATEntry."Add.-Curr. Realized Amount";
        VATEntry2."Add.-Curr. Realized Base" -= NewVATEntry."Add.-Curr. Realized Base";
    ...
    local procedure SkipFindTaxDetail(var TaxDetail: Record "Tax Detail"; GenJournalLine: Record "Gen. Journal Line"): Boolean
    var
        IsHandled: Boolean;
        ReturnValue: Boolean;
    begin
        OnBeforeSkipFindTaxDetail(TaxDetail, GenJournalLine, IsHandled, ReturnValue);
        if IsHandled then
            exit(ReturnValue);

        if GenJournalLine."VAT Posting" <> GenJournalLine."VAT Posting"::"Manual VAT Entry" then
            exit(false);
        if GenJournalLine."Gen. Posting Type" = GenJournalLine."Gen. Posting Type"::Settlement then
            exit(false);

        TaxDetail.Reset();
        TaxDetail.SetRange("Tax Jurisdiction Code", GenJournalLine."Tax Jurisdiction Code");
        TaxDetail.SetFilter("Tax Group Code", '%1|%2', '', GenJournalLine."Tax Group Code");
        TaxDetail.SetRange("Tax Type", GenJournalLine."Tax Type");
        if GenJournalLine."Tax Type" = GenJournalLine."Tax Type"::"Sales and Use Tax" then
            if GenJournalLine."Use Tax" then
        ...
        TaxDetail.SetRange("Effective Date", 0D, GenJournalLine."Posting Date");
    ...
    local procedure GetRealizedVATGainLossGLAccNo(VATEntry: Record "VAT Entry"; CurrencyCode: Code[10]): Code[20]
    var
        Currency: Record Currency;
    begin
        GetCurrency(Currency, CurrencyCode);
        if VATEntry."Realized Base" < 0 then
    ...
    [IntegrationEvent(false, false)]
    local procedure OnBeforeSkipFindTaxDetail(var TaxDetail: Record "Tax Detail"; var GenJournalLine: Record "Gen. Journal Line"; var IsHandled: Boolean; var ReturnValue: Boolean)
    begin
    end;
}
