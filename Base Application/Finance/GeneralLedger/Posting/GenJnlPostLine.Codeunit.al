...
codeunit 12 "Gen. Jnl.-Post Line"
{
    ...
    var
        ...
        Text1100009: Label '%1 cannot be applied, since it is included in a payment order.';
        Text1100010: Label 'Remove it from its payment order and try again.';
    ...
    begin
        ...
        GenJnlLine."Amount (LCY)" := Round(GenJnlLine."Amount (LCY)");
                                ...
                                begin
                                    ...
                                    if not IsHandled then
                                        if GenJnlLine."VAT Difference" <> 0 then begin
                                            ...
                                            GLEntry."VAT Amount" :=
                                              Round(
                                                GenJnlLine."Amount (LCY)" * VATPostingSetup."VAT+EC %" / (100 + VATPostingSetup."VAT+EC %"),
                                            ...
                                            if GenJnlLine."Source Currency Code" = AddCurrencyCode then
                                                AddCurrGLEntryVATAmt :=
                                                  Round(
                                                    GenJnlLine."Source Currency Amount" * VATPostingSetup."VAT+EC %" / (100 + VATPostingSetup."VAT+EC %"),
    ...
    begin
        UnrealizedVAT :=
            ...
            (IsNotPayment(GenJnlLine."Document Type") or (GenJnlLine."Document Type" = GenJnlLine."Document Type"::Bill));
    ...
    begin
        ...
        if not IsHandled then begin
            ...
            then
                CustLedgEntry."Pmt. Disc. Given (LCY)" := GenJournalLine."Pmt. Discount Given/Rec. (LCY)"
            else
                if GenJournalLine."Amount (LCY)" <> 0 then begin
                    if GLSetup."Pmt. Disc. Excl. VAT" then
                        CustLedgEntry."Original Pmt. Disc. Possible" := GenJournalLine."Sales/Purch. (LCY)" * GenJournalLine.Amount / GenJournalLine."Amount (LCY)"
                    else
                        CustLedgEntry."Original Pmt. Disc. Possible" := GenJournalLine.Amount;
                    CustLedgEntry."Original Pmt. Disc. Possible" :=
                    Round(
                        CustLedgEntry."Original Pmt. Disc. Possible" * GenJournalLine."Payment Discount %" / 100, AmountRoundingPrecision);
            ...
            OriginalEntryExist := IsOriginalEntryExist(GenJournalLine);

            // Post application
            OnPostCustOnBeforeApplyCustLedgEntry(GenJournalLine, CVLedgEntryBuf, FromBillSettlement);
            ApplyCustLedgEntry(CVLedgEntryBuf, TempDtldCVLedgEntryBuf, GenJournalLine, Cust);

            if (GenJournalLine."Applies-to ID" = '') and
            (GenJournalLine."Applies-to Doc. Type" = GenJournalLine."Applies-to Doc. Type"::" ")
            then
                GenJournalLine."Applies-to Doc. Type" := AppliesToDocType;

            if not GenJournalLine.Prepayment then begin
                if GenJournalLine."Document Type" = GenJournalLine."Document Type"::Bill then begin
                    DocPost.CreateReceivableDoc(GenJournalLine, CVLedgEntryBuf, BillFromJournal);
                ...
                SendDocToCartera(GenJournalLine, VendLedgEntry, CustLedgEntry, 0, CVLedgEntryBuf);
            end;
    ...
    begin
        ...
        if GLSetup."Payment Discount Type" =
           GLSetup."Payment Discount Type"::"Calc. Pmt. Disc. on Lines"
        then
            VendLedgEntry."Pmt. Disc. Rcd.(LCY)" := GenJournalLine."Pmt. Discount Given/Rec. (LCY)"
        else
            if GenJournalLine."Amount (LCY)" <> 0 then begin
                if GLSetup."Pmt. Disc. Excl. VAT" then
                    VendLedgEntry."Original Pmt. Disc. Possible" := GenJournalLine."Sales/Purch. (LCY)" * GenJournalLine.Amount / GenJournalLine."Amount (LCY)"
                else
                    VendLedgEntry."Original Pmt. Disc. Possible" := GenJournalLine.Amount;
                VendLedgEntry."Original Pmt. Disc. Possible" :=
                  Round(
                    VendLedgEntry."Original Pmt. Disc. Possible" * GenJournalLine."Payment Discount %" / 100, AmountRoundingPrecision);
        ...
        OriginalEntryExist := IsOriginalEntryExist(GenJournalLine);
        // Post application
        OnPostVendOnBeforeApplyVendLedgEntry(GenJournalLine, CVLedgEntryBuf, FromBillSettlement);
        ...
        if not GenJournalLine.Prepayment then begin
            if GenJournalLine."Document Type" = GenJournalLine."Document Type"::Bill then begin
                DocPost.CreatePayableDoc(GenJournalLine, CVLedgEntryBuf, BillFromJournal);
            ...
            SendDocToCartera(GenJournalLine, VendLedgEntry, CustLedgEntry, 1, CVLedgEntryBuf);
        end;
    ...
    begin
        ...
        if NextTransactionNoNeeded(GenJnlLine) then begin
            ...
            InitLastDocDate(GenJnlLine);
            FirstNewVATEntryNo := NextVATEntryNo;
            InitGLRegisterFromContinuePosting(GenJnlLine);
        end;
    ...
    local procedure InitGLRegisterFromContinuePosting(GenJnlLine: Record "Gen. Journal Line")
    begin
        LastTempTransNo := GenJnlLine."Transaction No.";
        ...
        OnAfterInitGLRegisterFromContinuePosting(GLReg, GenJnlLine);
    end;
    ...
    begin
        NewTransaction :=
            (LastDate <> GenJnlLine."Posting Date") or ForceDocBalance and (LastDocNo <> GenJnlLine."Document No.") or
            (LastTempTransNo <> GenJnlLine."Transaction No.") and (not ForceDocBalance);
    ...
    begin
        IsHandled := false;
        OnBeforeUpdateGLReg(IsTransactionConsistent, IsGLRegInserted, GLReg, IsHandled, GenJnlLine, GlobalGLEntry, FirstNewVATEntryNo);
        if IsHandled then
            exit;
    ...
    begin
        ...
        DtldCustLedgEntry."VAT Reporting Date" := GenJnlLine."VAT Reporting Date";
    ...
    begin
        ...
        DtldVendLedgEntry."VAT Reporting Date" := GenJnlLine."VAT Reporting Date";
    ...
    begin
        ...
        repeat
            ...
            if (OldCustLedgEntry."Document Type" = OldCustLedgEntry."Document Type"::Bill) or
               (OldCustLedgEntry."Document Situation" <> OldCustLedgEntry."Document Situation"::" ")
            then begin
                FromClosedDoc := GetCustomerFromClosedDoc(OldCustLedgEntry);
            ...
            if not IsHandled then
                ...
                then
                    if ((IsNotPayment(TempOldCustLedgEntry."Document Type") or
                         (TempOldCustLedgEntry."Document Type" = TempOldCustLedgEntry."Document Type"::Bill)) and
                        (NewCVLedgEntryBuf."Document Type" <> "Gen. Journal Document Type"::" ") and
    ...
    local procedure UpdateReceivableDocForNewCustLedgEntry(var GenJnlLine: Record "Gen. Journal Line"; var NewCVLedgEntryBuf: Record "CV Ledger Entry Buffer"; var NewCustLedgEntry: Record "Cust. Ledger Entry")
    begin
        OnBeforeUpdateReceivableDocForNewCustLedgEntry(GenJnlLine, NewCVLedgEntryBuf, NewCustLedgEntry);

        FromClosedDoc := false;
        if (NewCustLedgEntry."Document Type" = NewCustLedgEntry."Document Type"::Bill) or
           (NewCustLedgEntry."Document Situation" <> NewCustLedgEntry."Document Situation"::" ")
        then begin
            FromClosedDoc := GetCustomerFromClosedDoc(NewCustLedgEntry);
        ...
        OnAfterUpdateReceivableDocForNewCustLedgEntry(GenJnlLine, NewCVLedgEntryBuf, NewCustLedgEntry);
    end;
    ...
    begin
        ...
        if GenJnlLine."Applies-to Doc. No." <> '' then begin
            ...
            OldCustLedgEntry.SetLoadFields(Positive, "Posting Date", "Currency Code", "Document Situation");
            ...
            OldCustLedgEntry.Reset();
            OldCustLedgEntry.SetLoadFields("Posting Date", "Currency Code", "Applies-to ID", "Document Situation");
    ...
    begin
        ...
        if DtldCVLedgEntryBuf.FindSet() then begin
            ...
            OnPostDtldCustLedgEntriesOnAfterAssignReceivablesAccount(GenJnlLine, CustPostingGr, AccNo);

            CalcPostingBufferTotals(TempDimPostingBuffer);
            ReceivableAccAmtLCY :=
              TempDimPostingBuffer.Amount -
            ...
            ReceivableAccAmtAddCurr :=
              TempDimPostingBuffer."Amount (ACY)" -
            ...
            if CheckCarteraPostDtldCustLE(GenJnlLine, DtldCustLedgEntry2, ReceivableAccAmtLCY, ReceivableAccAmtAddCurr, false) then
                if (TempDimPostingBuffer.Amount <> 0) or ((TempDimPostingBuffer."Amount (ACY)" <> 0) and (AddCurrencyCode <> '')) or
                ...
                then
                    ...
                    then begin
                        InitGLEntry(
                            GenJnlLine, GLEntry, AccNo, ReceivableAccAmtLCY, ReceivableAccAmtAddCurr, true, true,
                            CalcAmountSrcCurr(GenJnlLine, ReceivableAccAmtLCY));
                        GLEntry."Bal. Account Type" := GenJnlLine."Bal. Account Type";
                        GLEntry."Bal. Account No." := GenJnlLine."Bal. Account No.";
    ...
    begin
        ...
        CheckCarteraCustUnrealizedVAT(
            VATEntry2, CustLedgEntry2, CustLedgEntry3, CustLedgEntry4, Doc, ClosedDoc, PostedDoc,
            FromDocInPostedBillGr, FromDoc);
        ...
        if VATEntry2.FindSet() and (not FromDocInPostedBillGr) then begin
            ...
            repeat
                ...
                OnCustUnrealizedVATOnAfterSetBillDoc(CustLedgEntry4, VATEntry2);
    ...
    local procedure CheckCarteraCustUnrealizedVAT(var VATEntry2: Record "VAT Entry"; var CustLedgEntry2: Record "Cust. Ledger Entry"; var CustLedgEntry3: Record "Cust. Ledger Entry"; var CustLedgEntry4: Record "Cust. Ledger Entry"; Doc: Record "Cartera Doc."; ClosedDoc: Record "Closed Cartera Doc."; PostedDoc: Record "Posted Cartera Doc."; FromDocInPostedBillGr: Boolean; var FromDoc: Boolean)
    var
        VATPostingSetup: Record "VAT Posting Setup";
        FromJnl: Boolean;
        IsHandled: Boolean;
    begin
        IsHandled := false;
        OnBeforeCheckCarteraCustUnrealizedVAT(VATEntry2, CustLedgEntry4, CustLedgEntry3, CustLedgEntry2, Doc, PostedDoc, ClosedDoc, IsHandled);
        if IsHandled then
            exit;
    ...
    begin
        ...
        repeat
            ...
            if (OldVendLedgEntry."Document Type" = OldVendLedgEntry."Document Type"::Bill) or
               (OldVendLedgEntry."Document Situation" <> OldVendLedgEntry."Document Situation"::" ")
            then begin
                FromClosedDoc := GetVendorFromClosedDoc(OldVendLedgEntry);
            ...
            OnAfterOldVendLedgEntryModify(OldVendLedgEntry, GenJnlLine, TempVendorLedgerEntry, DtldCVLedgEntryBuf);
            ...
            if not IsHandled then
                ...
                then
                    if (IsNotPayment(TempOldVendLedgEntry."Document Type") or
        ...
        UpdatePayableDocForNewVendLedgEntry(GenJnlLine, NewCVLedgEntryBuf, NewVendLedgEntry);
    ...
    local procedure UpdatePayableDocForNewVendLedgEntry(var GenJnlLine: Record "Gen. Journal Line"; var NewCVLedgEntryBuf: Record "CV Ledger Entry Buffer"; var NewVendLedgEntry: Record "Vendor Ledger Entry")
    begin
        OnBeforeUpdatePayableDocForNewVendLedgEntry(GenJnlLine, NewCVLedgEntryBuf, NewVendLedgEntry);

        FromClosedDoc := false;
        if (NewVendLedgEntry."Document Type" = NewVendLedgEntry."Document Type"::Bill) or
           (NewVendLedgEntry."Document Situation" <> NewVendLedgEntry."Document Situation"::" ")
        then begin
            FromClosedDoc := GetVendorFromClosedDoc(NewVendLedgEntry);
            DocPost.UpdatePayableDoc(
              NewVendLedgEntry, GenJnlLine, DocAmountLCY, AppliedAmountLCY2 + DeltaUnrealAmount + DeltaAmountLCY,
              DocLock, CollDocAmountLCY);
        end;

        OnAfterUpdatePayableDocForNewVendLedgEntry(GenJnlLine, NewCVLedgEntryBuf, NewVendLedgEntry);
    end;

    local procedure GetVendorFromClosedDoc(var VendorLedgerEntry: Record "Vendor Ledger Entry") Result: Boolean
    begin
        Result :=
            VendorLedgerEntry."Document Situation" in [VendorLedgerEntry."Document Situation"::"Closed Documents",
                                                       VendorLedgerEntry."Document Situation"::"Closed BG/PO"];
    end;

    local procedure GetCustomerFromClosedDoc(var CustLedgerEntry: Record "Cust. Ledger Entry") Result: Boolean
    begin
        Result :=
            CustLedgerEntry."Document Situation" in [CustLedgerEntry."Document Situation"::"Closed Documents",
    ...
    begin
        ...
        if GenJnlLine."Applies-to Doc. No." <> '' then begin
            ...
            OldVendLedgEntry.SetLoadFields(Positive, "Posting Date", "Currency Code", "Document Situation");
            ...
            // Find the first old entry (Invoice) which the new entry (Payment) should apply to
            OldVendLedgEntry.Reset();
            OldVendLedgEntry.SetLoadFields("Posting Date", "Currency Code", "Applies-to ID", "Document Situation");
    ...
    begin
        ...
        if DetailedCVLedgEntryBuffer.FindSet() then begin
            ...
            repeat
                ...
                UpdateVendLedgEntryStats(DetailedVendorLedgEntry."Vendor Ledger Entry No.");
                IsHandled := false;
                        ...
                        case DetailedCVLedgEntryBuffer."Entry Type" of
                            DetailedCVLedgEntryBuffer."Entry Type"::Application:
                                if CheckCarteraPostDtldVendLE(
                                    GenJournalLine, DetailedVendorLedgEntry, PayableAccAmtLCY, PayableAccAmtAddCurr, false)
                                then begin
                                    GetCurrency(Currency, DetailedCVLedgEntryBuffer."Currency Code");
                                    CheckNonAddCurrCodeOccurred(Currency.Code);
                                    CreateGLEntry(
                                        GenJournalLine, VendPostingGr.GetPayablesAccount(), -DetailedCVLedgEntryBuffer."Amount (LCY)", 0,
    ...
    local procedure GetVendCarteraAccountNo(GenJnlLine: Record "Gen. Journal Line"; VendPostingGr: Record "Vendor Posting Group") AccNo: Code[20]
    begin
        ...
        OnAfterGetVendCarteraAccountNo(GenJnlLine, VendPostingGr, AccNo);
    end;
    ...
    begin
        ...
        CalcPostingBufferTotals(TempDimPostingBuffer);
        PayableAccAmtLCY := TempDimPostingBuffer.Amount - (DocAmountLCY + CollDocAmountLCY);
        PayableAccAmtAddCurr :=
          TempDimPostingBuffer."Amount (ACY)" -
        ...
        if not IsHandled then
            CreateGLEntriesForTotalAmounts(
              GenJnlLine, TempDimPostingBuffer, AdjAmount, SaveEntryNo, EmplPostingGr.GetPayablesAccount());
                ...
                begin
                    if MultiplePostingGroups then
                        CreateGLEntry(
                            ...
                            CalcAmountSrcCurr(GenJournalLine, DetailedCVLedgEntryBuffer."Amount (LCY)"));
                    PostDtldCVApplicationEntry(GenJournalLine, DetailedCVLedgEntryBuffer);
                end;
    ...
    begin
        ...
        VATEntry2.Reset();
        VATEntry2.SetCurrentKey("Transaction No.");
        CheckCarteraVendUnrealizedVAT(
            VATEntry2, VendLedgEntry2, VendLedgEntry3, VendLedgEntry4,
            Doc, ClosedDoc, PostedDoc, FromDocInPostedPmtOrd);
    ...
    local procedure CheckCarteraVendUnrealizedVAT(var VATEntry2: Record "VAT Entry"; var VendLedgEntry2: Record "Vendor Ledger Entry"; var VendLedgEntry3: Record "Vendor Ledger Entry"; var VendLedgEntry4: Record "Vendor Ledger Entry"; Doc: Record "Cartera Doc."; ClosedDoc: Record "Closed Cartera Doc."; PostedDoc: Record "Posted Cartera Doc."; FromDocInPostedPmtOrd: Boolean)
    var
        VATPostingSetup: Record "VAT Posting Setup";
        FromJnl: Boolean;
        IsHandled: Boolean;
    begin
        IsHandled := false;
        OnBeforeCheckCarteraVendUnrealizedVAT(VATEntry2, VendLedgEntry4, VendLedgEntry3, VendLedgEntry2, Doc, PostedDoc, ClosedDoc, IsHandled);
        if IsHandled then
            exit;
    ...
    begin
        ...
        if DetailedCustLedgEntry."Transaction No." <> 0 then begin
            ...
            repeat
                CheckCarteraAccessPermissions(DetailedCustLedgEntry2."Document Situation");
        ...
        repeat
            ...
            if not IsHandled then begin
                ...
                if not IsHandled then begin
                    ...
                    if DetailedCVLedgEntryBuffer."Entry Type" = DetailedCVLedgEntryBuffer."Entry Type"::Application then
                        if DetailedCVLedgEntryBuffer."Amount (LCY)" >= 0 then begin
                            PositiveLCYAppAmt := PositiveLCYAppAmt + DetailedCVLedgEntryBuffer."Amount (LCY)";
                            PositiveACYAppAmt :=
                              PositiveACYAppAmt + DetailedCVLedgEntryBuffer."Additional-Currency Amount";
                        end else begin
                            NegativeLCYAppAmt := NegativeLCYAppAmt + DetailedCVLedgEntryBuffer."Amount (LCY)";
                            NegativeACYAppAmt :=
                              NegativeACYAppAmt + DetailedCVLedgEntryBuffer."Additional-Currency Amount";
                        end;
                ...
                UpdateCustLedgEntry(DetailedCustLedgEntry2, GenJournalLineToPost);
        ...
        if IsTempGLEntryBufEmpty() then begin
            if GenJournalLineToPost."Financial Void" then
                GLAccountNo := CustomerPostingGroup.GetReceivablesAccount()
            else
                if CheckDtldCustLegderEntryCreateBills(DetailedCustLedgEntry) then
                    GLAccountNo := CustomerPostingGroup.GetBillsAccount(false)
                else
                    GLAccountNo := CustomerPostingGroup.GetReceivablesAccount();
            InitGLEntry(
                GenJournalLineToPost, GLEntry, GLAccountNo, PositiveLCYAppAmt, PositiveACYAppAmt, false, true,
                CalcAmountSrcCurr(GenJournalLineToPost, PositiveLCYAppAmt));
            InsertGLEntry(GenJournalLineToPost, GLEntry, false);
            InitGLEntry(
                GenJournalLineToPost, GLEntry, CustomerPostingGroup."Receivables Account", NegativeLCYAppAmt, NegativeACYAppAmt, false, true,
                CalcAmountSrcCurr(GenJournalLineToPost, NegativeLCYAppAmt));
            InsertGLEntry(GenJournalLineToPost, GLEntry, false);
        end;
    ...
    begin
        ...
        if DetailedVendorLedgEntry."Transaction No." <> 0 then begin
            ...
            repeat
                CheckCarteraAccessPermissions(DetailedVendorLedgEntry2."Document Situation");
        ...
        repeat
            ...
            UpdateVendLedgEntry(DetailedVendorLedgEntry2, GenJournalLineToPost);
        until DetailedVendorLedgEntry2.Next() = 0;
        ...
        if IsTempGLEntryBufEmpty() then begin
            if FinancialVoidWithMethodCreateBills(GenJournalLineToPost) then
                GLAccountNo := VendorPostingGroup.GetBillsAccount()
            else
                if GenJournalLineToPost."Financial Void" then
                    GLAccountNo := VendorPostingGroup.GetPayablesAccount()
                else
                    if CheckDtldVendLegderEntryCreateBills(DetailedVendorLedgEntry) then
                        GLAccountNo := VendorPostingGroup.GetBillsAccount()
                    else
                        GLAccountNo := VendorPostingGroup.GetPayablesAccount();
            InitGLEntry(
                GenJournalLineToPost, GLEntry, VendorPostingGroup.GetPayablesAccount(), PositiveLCYAppAmt, PositiveACYAppAmt, false, true,
                CalcAmountSrcCurr(GenJournalLineToPost, PositiveLCYAppAmt));
            InsertGLEntry(GenJournalLineToPost, GLEntry, false);
            InitGLEntry(
                GenJournalLineToPost, GLEntry, GLAccountNo, NegativeLCYAppAmt, NegativeACYAppAmt, false, true,
                CalcAmountSrcCurr(GenJournalLineToPost, NegativeLCYAppAmt));
            InsertGLEntry(GenJournalLineToPost, GLEntry, false);
        end;
        CheckPostUnrealizedVAT(GenJournalLineToPost, true);
    ...
    begin
        ...
        NewDtldCustLedgEntry."VAT Reporting Date" := GenJnlLine."VAT Reporting Date";
    ...
    begin
        ...
        NewDtldVendLedgEntry."VAT Reporting Date" := GenJnlLine."VAT Reporting Date";
    ...
    local procedure UpdateCustLedgEntry(DtldCustLedgEntry: Record "Detailed Cust. Ledg. Entry"; GenJnlLine: Record "Gen. Journal Line")
    var
        CustLedgEntry: Record "Cust. Ledger Entry";
    begin
    ...
    local procedure UpdateVendLedgEntry(DtldVendLedgEntry: Record "Detailed Vendor Ledg. Entry"; GenJnlLine: Record "Gen. Journal Line")
    var
        VendLedgEntry: Record "Vendor Ledger Entry";
    begin
    ...
    local procedure PostPayableDocs(GenJnlLine: Record "Gen. Journal Line"; VendPostingGr: Record "Vendor Posting Group")
    var
        GLAccNo: Code[20];
    begin
    ...
    [Scope('OnPrem')]
    procedure DocAmtCalcAddCurrency(GenJnlLine: Record "Gen. Journal Line"; Amount: Decimal): Decimal
    begin
        if (AddCurrencyCode <> '') and
           (GenJnlLine."Additional-Currency Posting" = GenJnlLine."Additional-Currency Posting"::None)
        then
    ...
    begin
        IsHandled := false;
        OnBeforeVendFindVATSetup(VATSetup, VendLedgEntry4, IsFromJournal, ExistsVATNoReal, IsHandled);
        if IsHandled then
        ...
        if IsFromJournal then begin
            ...
            FindFirstPurchInvoiceLine(PurchLine2, VendLedgEntry4);
            ...
            repeat
                ...
                OnVendFindVATSetupOnAfterPurchLine2Loop(PurchLine2, ProdPostingGroup, ErrorMessage, ExistsVATNoReal);
    ...
    local procedure FindFirstPurchInvoiceLine(var PurchInvLine: Record "Purch. Inv. Line"; var VendLedgEntry4: Record "Vendor Ledger Entry")
    var
        IsHandled: Boolean;
    begin
        IsHandled := false;
        OnBeforeFindFirstPurchInvoiceLine(PurchInvLine, VendLedgEntry4, IsHandled);
        if IsHandled then
            exit;
    ...
    procedure CustFindVATSetup(var VATSetup: Record "VAT Posting Setup"; CustLedgEntry4: Record "Cust. Ledger Entry"; IsFromJournal: Boolean): Boolean
    var
        Customer: Record Customer;
        ProdPostingGroup: Code[20];
    ...
    local procedure CheckCarteraPostDtldCustLE(GenJnlLine2: Record "Gen. Journal Line"; DtldCustLedgEntry3: Record "Detailed Cust. Ledg. Entry"; ReceivableAccAmtLCY: Decimal; ReceivableAccAmtAddCurr: Decimal; CreateBills: Boolean): Boolean
    var
        IsOK: Boolean;
    begin
        IsOK := false;
        ...
        case DtldCustLedgEntry3."Entry Type" of
            DtldCustLedgEntry3."Entry Type"::"Initial Entry":
                if DtldCustLedgEntry3."Initial Document Type" = DtldCustLedgEntry3."Initial Document Type"::Bill then
                    if DtldCustLedgEntry3."Document Type" = DtldCustLedgEntry3."Document Type"::Bill then
                        if BillFromJournal then
                            IsOK := true;
            DtldCustLedgEntry3."Entry Type"::Application:
                if (not FromBillSettlement) and (not IDBillSettlement) then
                    if ((DtldCustLedgEntry3."Document Type" <> DtldCustLedgEntry3."Document Type"::"Credit Memo") and (GenJnlLine2."Applies-to ID" <> '')) or
                       CreateBills
                    then begin
                        if DtldCustLedgEntry3."Initial Document Type" = DtldCustLedgEntry3."Initial Document Type"::Bill then
                            IsOK := true;
                        if DtldCustLedgEntry3."Initial Document Type" = DtldCustLedgEntry3."Initial Document Type"::Invoice then
                            if DtldCustLedgEntry3."Document Situation" = DtldCustLedgEntry3."Document Situation"::"Closed BG/PO" then
                                if DtldCustLedgEntry3."Document Status" = DtldCustLedgEntry3."Document Status"::Rejected then
                                    IsOK := true;
                    end;
            else
        ...
        OnAfterCheckCarteraPostDtldCustLE(GenJnlLine2, DtldCustLedgEntry3, IsOK);
        exit(IsOK);
    end;

    local procedure CheckCarteraPostDtldVendLE(GenJnlLine2: Record "Gen. Journal Line"; DtldVendLedgEntry3: Record "Detailed Vendor Ledg. Entry"; PayableAccAmtLCY: Decimal; PayableAccAmtAddCurr: Decimal; CreateBills: Boolean): Boolean
    var
        IsOK: Boolean;
    begin
        IsOK := false;
        ...
        case DtldVendLedgEntry3."Entry Type" of
            DtldVendLedgEntry3."Entry Type"::Application:
                if (not FromBillSettlement) and (not IDBillSettlement) then
                    if ((DtldVendLedgEntry3."Document Type" <> DtldVendLedgEntry3."Document Type"::"Credit Memo") and (GenJnlLine2."Applies-to ID" <> '')) or
                       CreateBills
                    then
                        if DtldVendLedgEntry3."Initial Document Type" = DtldVendLedgEntry3."Initial Document Type"::Bill then
                            IsOK := true;
            else
        ...
        OnAfterCheckCarteraPostDtldVendLE(GenJnlLine2, DtldVendLedgEntry3, IsOK);
        exit(IsOK);
    end;

    [Scope('OnPrem')]
    procedure IsOriginalEntryExist(GenJnlLine: Record "Gen. Journal Line"): Boolean
    var
    ...
    begin
        ...
        if GenJnlLine."Applies-to ID" <> '' then begin
            if GenJnlLine."Account Type" = GenJnlLine."Account Type"::Customer then begin
                ...
                if not CustLedgerEntry1.IsEmpty() then
                    IsCreditMemo := true;
                CustLedgerEntry1.Reset();
                CustLedgerEntry1.SetRange("Applies-to ID", GenJnlLine."Applies-to ID");
            ...
            if GenJnlLine."Account Type" = GenJnlLine."Account Type"::Vendor then begin
                ...
                if not VendLedgerEntry1.IsEmpty() then
                    exit(true);
            end;
        end;
    end;
    ...
    local procedure GetCustOriginalAmtLCY(UnrealCustLedgEntry: Record "Cust. Ledger Entry"; BillCustLedgEntry: Record "Cust. Ledger Entry"; InvCustLedgEntry: Record "Cust. Ledger Entry"): Decimal
    begin
    ...
    local procedure GetVendSettledAmount(SettledAmt: Decimal; UnrealVendLedgEntry: Record "Vendor Ledger Entry"; BillVendLedgEntry: Record "Vendor Ledger Entry"; InvVendLedgEntry: Record "Vendor Ledger Entry"): Decimal
    begin
        if IsVendBillDoc(UnrealVendLedgEntry) then
    ...
    local procedure GetCustSettledAmount(SettledAmt: Decimal; UnrealCustLedgEntry: Record "Cust. Ledger Entry"; BillCustLedgEntry: Record "Cust. Ledger Entry"; InvCustLedgEntry: Record "Cust. Ledger Entry"): Decimal
    begin
        if IsCustBillDoc(UnrealCustLedgEntry) then
    ...
    local procedure GetGLAccountForPayablesDocs(GenJnlLine: Record "Gen. Journal Line"; VendPostingGr: Record "Vendor Posting Group"; DocAmountLCY: Decimal; CollDocAmountLCY: Decimal) AccountNo: Code[20]
    var
        IsHandled: Boolean;
    begin
        IsHandled := false;
        OnBeforeGetGLAccountForPayablesDocs(GenJnlLine, VendPostingGr, DocAmountLCY, AccountNo, IsHandled);
        if IsHandled then
            exit(AccountNo);
    ...
    local procedure GetGLAccountForReceivableDocs(GenJnlLine: Record "Gen. Journal Line"; CustPostingGr: Record "Customer Posting Group"; DocAmountLCY: Decimal; CollDocAmountLCY: Decimal) AccountNo: Code[20]
    var
        IsHandled: Boolean;
    begin
        IsHandled := false;
        OnBeforeGetGLAccountForReceivableDocs(GenJnlLine, CustPostingGr, DocAmountLCY, AccountNo, IsHandled);
        if IsHandled then
            exit(AccountNo);
    ...
    local procedure IsVendBillDoc(UnrealVendLedgEntry: Record "Vendor Ledger Entry") IsBillDoc: Boolean
    var
        IsHandled: Boolean;
    begin
        IsHandled := false;
        OnBeforeIsVendBillDoc(UnrealVendLedgEntry, IsBillDoc, IsHandled);
        if IsHandled then
            exit(IsBillDoc);
    ...
    local procedure IsCustBillDoc(UnrealCustLedgEntry: Record "Cust. Ledger Entry") IsBillDoc: Boolean
    var
        IsHandled: Boolean;
    begin
        IsHandled := false;
        OnBeforeIsCustBillDoc(UnrealCustLedgEntry, IsBillDoc, IsHandled);
        if IsHandled then
            exit(IsBillDoc);
    ...
    local procedure CalcVendRealizedVATAmt(var VATBase: Decimal; var VATAmount: Decimal; var VATBaseAddCurr: Decimal; var VATAmountAddCurr: Decimal; VATEntry2: Record "VAT Entry"; UnrealVendLedgEntry: Record "Vendor Ledger Entry"; VATPart: Decimal)
    begin
        CalcRealizedVATAmt(
          VATBase, VATAmount,
    ...
    local procedure CalcRealizedVATAmt(var VATBase: Decimal; var VATAmount: Decimal; var VATBaseAddCurr: Decimal; var VATAmountAddCurr: Decimal; VATEntry2: Record "VAT Entry"; UseUnrealAmt: Boolean; VATPart: Decimal)
    var
        UnrealAmt: Decimal;
        UnrealBase: Decimal;
    ...
    begin
        if UseUnrealAmt then begin
            UnrealAmt := VATEntry2."Unrealized Amount";
            UnrealBase := VATEntry2."Unrealized Base";
            UnrealAmtACY := VATEntry2."Add.-Currency Unrealized Amt.";
            UnrealBaseACY := VATEntry2."Add.-Currency Unrealized Base";
        end else begin
            UnrealAmt := VATEntry2."Remaining Unrealized Amount";
            UnrealBase := VATEntry2."Remaining Unrealized Base";
            UnrealAmtACY := VATEntry2."Add.-Curr. Rem. Unreal. Amount";
            UnrealBaseACY := VATEntry2."Add.-Curr. Rem. Unreal. Base";
        end;
    ...
    local procedure CalcPostingBufferTotals(var TempDimPostingBuffer: Record "Dimension Posting Buffer" temporary)
    var
        TotalAmount: Decimal;
    ...
    begin
        ...
        if TempDimPostingBuffer.FindSet() then
            repeat
                TotalAmount += TempDimPostingBuffer.Amount;
                TotalAmountACY += TempDimPostingBuffer."Amount (ACY)";
            until TempDimPostingBuffer.Next() = 0;
        TempDimPostingBuffer.Amount := TotalAmount;
        TempDimPostingBuffer."Amount (ACY)" := TotalAmountACY;
    end;
    ...
    var
        CustLedgEntry: Record "Cust. Ledger Entry";
    begin
        CustLedgEntry.SetCurrentKey(CustLedgEntry."Document No.", CustLedgEntry."Bill No.");
        CustLedgEntry.SetRange(CustLedgEntry."Document No.", DocNo);
        CustLedgEntry.SetRange(CustLedgEntry."Bill No.", BillNo);
        if CustLedgEntry.FindFirst() then
            if (CustLedgEntry."Document Status" = CustLedgEntry."Document Status"::Rejected) or
    ...
    begin
        IsHandled := false;
        OnBeforeCreateGLEntriesForTotalAmountsV19(TempDimPostingBuffer, GenJnlLine, GLAccNo, IsHandled, AdjAmountBuf, SavedEntryNo);
        if IsHandled then
            exit;

        if (TempDimPostingBuffer.Amount = 0) and (TempDimPostingBuffer."Amount (ACY)" = 0) and (GenJnlLine."Applies-to ID" = '') then begin
            IsHandled := false;
    ...
    local procedure FinancialVoidWithMethodCreateBills(GenJnlLine: Record "Gen. Journal Line"): Boolean
    var
        Vendor: Record Vendor;
    begin
        if not (GenJnlLine."Financial Void" and (GenJnlLine."Account Type" = GenJnlLine."Account Type"::Vendor)) then
            exit(false);
    ...
    local procedure CheckDtldCustLegderEntryCreateBills(InitDtldCustLedgEntry: Record "Detailed Cust. Ledg. Entry"): Boolean
    var
        DtldCustLedgEntry: Record "Detailed Cust. Ledg. Entry";
    begin
        ...
        if InitDtldCustLedgEntry."Application No." <> 0 then begin
            DtldCustLedgEntry.Get(InitDtldCustLedgEntry."Application No.");
            if not (DtldCustLedgEntry."Initial Document Type" in [DtldCustLedgEntry."Initial Document Type"::Invoice, DtldCustLedgEntry."Initial Document Type"::Bill]) then
                exit(false);
        end else begin
            ...
            if not DtldCustLedgEntry.FindFirst() then
                DtldCustLedgEntry.SetRange(DtldCustLedgEntry."Initial Document Type", DtldCustLedgEntry."Initial Document Type"::Invoice);
            if not DtldCustLedgEntry.FindFirst() then
                exit(false);
        end;
        exit(CheckCustLegderEntryCreateBills(DtldCustLedgEntry."Cust. Ledger Entry No."));
    end;
    ...
    local procedure CheckDtldVendLegderEntryCreateBills(InitDtldVendorLedgEntry: Record "Detailed Vendor Ledg. Entry"): Boolean
    var
    ...
    begin
        ...
        if InitDtldVendorLedgEntry."Application No." <> 0 then begin
            DtldVendorLedgEntry.Get(InitDtldVendorLedgEntry."Application No.");
            if not (DtldVendorLedgEntry."Initial Document Type" in [DtldVendorLedgEntry."Initial Document Type"::Invoice, DtldVendorLedgEntry."Initial Document Type"::Bill]) then
                exit(false);
        end else begin
            DtldVendorLedgEntry.SetRange(DtldVendorLedgEntry."Transaction No.", InitDtldVendorLedgEntry."Transaction No.");
            DtldVendorLedgEntry.SetRange(DtldVendorLedgEntry."Entry Type", DtldVendorLedgEntry."Entry Type"::Application);
            DtldVendorLedgEntry.SetRange(DtldVendorLedgEntry."Initial Document Type", DtldVendorLedgEntry."Initial Document Type"::Bill);
            if not DtldVendorLedgEntry.FindFirst() then
                DtldVendorLedgEntry.SetRange(DtldVendorLedgEntry."Initial Document Type", DtldVendorLedgEntry."Initial Document Type"::Invoice);
            if not DtldVendorLedgEntry.FindFirst() then
                exit(false);
        end;
        exit(CheckVendLegderEntryCreateBills(DtldVendorLedgEntry."Vendor Ledger Entry No."));
    end;
    ...
    local procedure UseGLBillsAccount(GenJnlLine: Record "Gen. Journal Line"): Boolean
    begin
        if (GenJnlLine."Document Type" = GenJnlLine."Document Type"::Bill) or FinancialVoidWithMethodCreateBills(GenJnlLine) then
            exit(true);

        if (GenJnlLine."Applies-to Doc. Type" = GenJnlLine."Applies-to Doc. Type"::Bill) and (GenJnlLine."Applies-to Doc. No." <> '') then
            if GenJnlLine."Document Type" = GenJnlLine."Document Type"::"Credit Memo" then
                exit((GenJnlLine."Applies-to Bill No." = '') and GenJnlLine."System-Created Entry")
            else
                exit(GenJnlLine."Applies-to Bill No." <> '')
        else
            exit(false);
    end;

    local procedure PostDtldCVApplicationEntry(GenJnlLine: Record "Gen. Journal Line"; DtldCVLedgEntryBuf: Record "Detailed CV Ledg. Entry Buffer")
    begin
        case GenJnlLine."Account Type" of
            GenJnlLine."Account Type"::Customer:
    ...
    local procedure PostDtldCustApplicationEntry(GenJnlLine: Record "Gen. Journal Line"; DtldCVLedgEntryBuf: Record "Detailed CV Ledg. Entry Buffer")
    var
        Currency: Record Currency;
        GLEntry: Record "G/L Entry";
        DtldCustLedgEntry: Record "Detailed Cust. Ledg. Entry";
    ...
    begin
        ...
        if CheckCarteraPostDtldCustLE(GenJnlLine, DtldCustLedgEntry, 0, 0, CreateBills) then begin
            ...
            InitGLEntry(
                GenJnlLine, GLEntry, AccNo, -DtldCVLedgEntryBuf."Amount (LCY)", 0, DtldCVLedgEntryBuf."Currency Code" = AddCurrencyCode, true,
                CalcAmountSrcCurr(GenJnlLine, -DtldCVLedgEntryBuf."Amount (LCY)"));
            InsertGLEntry(GenJnlLine, GLEntry, true);
    ...
    local procedure PostDtldVendApplicationEntry(GenJnlLine: Record "Gen. Journal Line"; DtldCVLedgEntryBuf: Record "Detailed CV Ledg. Entry Buffer")
    var
        Currency: Record Currency;
        GLEntry: Record "G/L Entry";
        DtldVendLedgEntry: Record "Detailed Vendor Ledg. Entry";
    ...
    begin
        ...
        if CheckCarteraPostDtldVendLE(GenJnlLine, DtldVendLedgEntry, 0, 0, CreateBills) then begin
            ...
            InitGLEntry(
                GenJnlLine, GLEntry, AccNo, -DtldCVLedgEntryBuf."Amount (LCY)", 0, DtldCVLedgEntryBuf."Currency Code" = GLSetup."Additional Reporting Currency", true,
                CalcAmountSrcCurr(GenJnlLine, -DtldCVLedgEntryBuf."Amount (LCY)"));
            InsertGLEntry(GenJnlLine, GLEntry, true);
    ...
    [IntegrationEvent(false, false)]
    local procedure OnAfterCheckCarteraPostDtldCustLE(GenJournalLine: Record "Gen. Journal Line"; DtldCustLedgEntry3: Record "Detailed Cust. Ledg. Entry"; var IsOk: Boolean)
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnAfterCheckCarteraPostDtldVendLE(GenJournalLine: Record "Gen. Journal Line"; DtldVendLedgEntry3: Record "Detailed Vendor Ledg. Entry"; var IsOk: Boolean)
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnAfterInitGLRegisterFromContinuePosting(var GLRegister: Record "G/L Register"; GenJournalLine: Record "Gen. Journal Line")
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnBeforeIsCustBillDoc(CustLedgerEntry: Record "Cust. Ledger Entry"; var IsBillDoc: Boolean; var IsHandled: Boolean)
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnBeforeIsVendBillDoc(UnrealVendLedgEntry: Record "Vendor Ledger Entry"; var IsBillDoc: Boolean; var IsHandled: Boolean)
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnBeforeGetGLAccountForPayablesDocs(GenJournalLine: Record "Gen. Journal Line"; VendPostingGr: Record "Vendor Posting Group"; DocAmountLCY: Decimal; var AccountNo: Code[20]; var IsHandled: Boolean)
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnBeforeUpdateReceivableDocForNewCustLedgEntry(var GenJournalLine: Record "Gen. Journal Line"; var CVLedgerEntryBuffer: Record "CV Ledger Entry Buffer"; var CustLedgerEntry: Record "Cust. Ledger Entry")
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnBeforeUpdatePayableDocForNewVendLedgEntry(var GenJournalLine: Record "Gen. Journal Line"; var CVLedgerEntryBuffer: Record "CV Ledger Entry Buffer"; var VendorLedgerEntry: Record "Vendor Ledger Entry")
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnAfterUpdateReceivableDocForNewCustLedgEntry(var GenJournalLine: Record "Gen. Journal Line"; var CVLedgerEntryBuffer: Record "CV Ledger Entry Buffer"; var CustLedgerEntry: Record "Cust. Ledger Entry")
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnAfterUpdatePayableDocForNewVendLedgEntry(var GenJournalLine: Record "Gen. Journal Line"; var CVLedgerEntryBuffer: Record "CV Ledger Entry Buffer"; var VendorLedgerEntry: Record "Vendor Ledger Entry")
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnPostCustOnBeforeApplyCustLedgEntry(var GenJournalLine: Record "Gen. Journal Line"; var CVLedgerEntryBuffer: Record "CV Ledger Entry Buffer"; var FromBillSettlement: Boolean)
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnPostVendOnBeforeApplyVendLedgEntry(var GenJournalLine: Record "Gen. Journal Line"; var CVLedgerEntryBuffer: Record "CV Ledger Entry Buffer"; var FromBillSettlement: Boolean)
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnCustUnrealizedVATOnAfterSetBillDoc(var CustLedgerEntry: Record "Cust. Ledger Entry"; var VATEntry: Record "VAT Entry")
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnAfterGetVendCarteraAccountNo(GenJournalLine: Record "Gen. Journal Line"; VendorPostingGroup: Record "Vendor Posting Group"; var AccountNo: Code[20])
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnVendFindVATSetupOnAfterPurchLine2Loop(PurchLine2: Record "Purch. Inv. Line"; var ProdPostingGroup: Code[20]; var ErrorMessage: Boolean; var ExistsVATNoReal: Boolean)
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnPostDtldCustLedgEntriesOnAfterAssignReceivablesAccount(GenJnlLine: Record "Gen. Journal Line"; CustomerPostingGroup: Record "Customer Posting Group"; var AccNo: Code[20])
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnBeforeVendFindVATSetup(var VATPostingSetup: Record "VAT Posting Setup"; VendorLedgerEntry4: Record "Vendor Ledger Entry"; IsFromJournal: Boolean; var ExistsVATNoReal: Boolean; var IsHandled: Boolean)
    begin
    end;
