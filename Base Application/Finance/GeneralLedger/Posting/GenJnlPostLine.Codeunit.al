...
codeunit 12 "Gen. Jnl.-Post Line"
{
    ...
    begin
        ...
        if GLEntryVATAmountNotEmpty or (PropDeductionVAT <> 0) or
    ...
    begin
        if not NonDeductibleVAT.IsNonDeductibleVATEnabled() then begin
            ...
            OnInsertVATOnBeforeCreateGLEntryForReverseChargeVATToRevChargeAcc(
                GenJnlLine, VATPostingSetup, VATPostingParameters."Unrealized VAT", VATPostingParameters."Full VAT Amount", VATPostingParameters."Full VAT Amount ACY", true);
            CreateGLEntry(GenJnlLine, VATPostingSetup.GetRevChargeAccount(VATPostingParameters."Unrealized VAT"),
    ...
    begin
        ...
        if not IsHandled then begin
            ...
            GLEntry.Amount := OrigGLEntryAmount;
    ...
    begin
        ...
        if GenJournalLine."Waiting Journal Reference" <> 0 then
            RemTools.SettleWaitingJournal(GenJournalLine);
    ...
    local procedure GLEntryAdjustPurchase(var GLEntry: Record "G/L Entry"; GenJnlLine: Record "Gen. Journal Line"; VATPostingSetup: Record "VAT Posting Setup")
    begin
    ...
    local procedure IsPropDeduction(Amount: Decimal; GenJnlLine: Record "Gen. Journal Line"; VATPostingSetup: Record "VAT Posting Setup"): Boolean
    begin
        exit(
    ...
    [Scope('OnPrem')]
    procedure GetReverseChargeVATAmount(Amount: Decimal; PropDeductionAmount: Decimal; UnrealizedVAT: Boolean): Decimal
    begin
    ...
    [Scope('OnPrem')]
    procedure GetReverseChargeVATAmountACY(AmountACY: Decimal; PropDeductionAmountACY: Decimal; UnrealizedVAT: Boolean): Decimal
    begin
        if UnrealizedVAT then
            exit(-AmountACY);
