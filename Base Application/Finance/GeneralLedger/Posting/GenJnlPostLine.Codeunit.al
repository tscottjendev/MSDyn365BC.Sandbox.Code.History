...
codeunit 12 "Gen. Jnl.-Post Line"
{
    ...
    begin
        ...
        if GenJnlLine."Gen. Posting Type" <> GenJnlLine."Gen. Posting Type"::" " then begin
            ...
            if (not GLSetup."Unrealized VAT") and (VATEntry.Type <> VATEntry.Type::Settlement) then begin
                VATEntry."Currency Code" := GenJnlLine."Source Currency Code";
                VATEntry."Amount (FCY)" := GenJnlLine."Source Curr. VAT Amount";
                VATEntry."Base (FCY)" := GenJnlLine."Source Curr. VAT Base Amount";
                VATEntry."Currency Factor" := GenJnlLine."Currency Factor";
                if VATPostingSetup."VAT Calculation Type" = GenJnlLine."VAT Calculation Type"::"Reverse Charge VAT" then begin
                    GetCurrency(Currency, GenJnlLine."Currency Code");
                    VATEntry."VAT %" := VATPostingSetup."VAT %";
                    if (not GenJnlLine."System-Created Entry") and (VATEntry."Amount (FCY)" = 0) then
                        VATEntry."Amount (FCY)" := Round(GenJnlLine.Amount * VATEntry."VAT %" / 100, Currency."Amount Rounding Precision");
                end else
                    VATEntry."VAT %" := GenJnlLine."VAT %";
                if VATEntry."Currency Code" <> '' then
    ...
    begin
        ...
        if DTASetup.ReadPermission then
            DTAMgt.CheckVenorPost(GenJournalLine);
    ...
    begin
        ...
        if GenJnlLine."Amount (LCY)" <> 0 then begin
            ...
            CVLedgEntryBuf."Original Pmt. Disc. Possible" :=
              GLSetup.RoundPmtDiscLCY(GenJnlLine.Amount, CVLedgEntryBuf."Original Pmt. Disc. Possible", GenJnlLine."Currency Code");
    ...
    begin
        ...
        VATEntry2.SetRange("Exchange Rate Adjustment", false);
    ...
    begin
        ...
        if not GLSetup."Unrealized VAT" then begin
            TempVATEntry."Currency Factor" := GenJnlLine."Currency Factor";
            TempVATEntry."Currency Code" := GenJnlLine."Source Currency Code";
            if TempVATEntry."Currency Code" <> '' then begin
                ApplnCurrency.Get(TempVATEntry."Currency Code");
                TempVATEntry."Amount (FCY)" :=
                  Round(VATAmount * TempVATEntry."Currency Factor", ApplnCurrency."Amount Rounding Precision");
                TempVATEntry."Base (FCY)" :=
                  Round(VATBase * TempVATEntry."Currency Factor", ApplnCurrency."Amount Rounding Precision");
            end;
            if TempVATEntry."Currency Code" <> '' then
                TempVATEntry."Unadjusted Exchange Rate" := true
            else
                TempVATEntry."Unadjusted Exchange Rate" := false;
        end;
    ...
    begin
        ...
        GenJnlLine."Currency Factor" := CustLedgEntryPostApply."Original Currency Factor";
    ...
    begin
        ...
        NewCVLedgEntryBuf."Remaining Pmt. Disc. Possible" :=
          GLSetup.RoundPmtDiscLCY(
            NewCVLedgEntryBuf."Remaining Amount", NewCVLedgEntryBuf."Remaining Pmt. Disc. Possible",
    ...
    begin
        ...
        GenJnlLine."Currency Factor" := VendLedgEntryPostApply."Original Currency Factor";
