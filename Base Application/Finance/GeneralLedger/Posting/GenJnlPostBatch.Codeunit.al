...
#pragma warning restore AL0432
    ...
    begin
        ...
        repeat
            ...
            if not IsHandled then begin
                ...
                if not IsHandled then
                    if ForceCheckBalance or (GenJnlLine."Posting Date" <> LastDate) or
                    (GenJnlTemplate."Force Doc. Balance" and ((GenJnlLine."Document Type" <> LastDocType) or (GenJnlLine."Document No." <> LastDocNo))) or
                    (not GenJnlTemplate."Force Doc. Balance" and (GenJnlLine."Transaction No." <> LastTempTransNo))
                    then begin
                        CheckBalance(GenJnlLine);
                        if (GenJnlLine."Posting Date" <> LastDate) or
    ...
    begin
        IsHandled := false;
        OnBeforeCheckCorrection(GenJournalLine, IsHandled, GenJnlTemplate, LastDate, LastDocType, LastDocNo, LastTempTransNo, DocCorrection);
        if IsHandled then
            exit;
        ...
        if (GenJournalLine."Posting Date" <> LastDate) or (GenJournalLine."Document Type" <> LastDocType) or (GenJournalLine."Document No." <> LastDocNo) or
           (GenJournalLine."Transaction No." <> LastTempTransNo)
        then begin
            ...
            if GenJournalLine.Correction <> DocCorrection then
                GenJournalLine.FieldError(Correction, Text1100001);
    end;
    ...
    begin
        ...
        if AllocateGenJnlLine."Account No." <> '' then begin
            ...
            if GenJnlAlloc.FindSet(true) then begin
                ...
                GenJnlLine2."Transaction No." := AllocateGenJnlLine."Transaction No.";
    ...
    [Scope('OnPrem')]
    procedure TestNoCustVend(GenJnlLine6: Record "Gen. Journal Line"; CurrGenJnlLine: Record "Gen. Journal Line") NoCustVend: Code[20]
    var
        GenJnlLine7: Record "Gen. Journal Line";
    begin
        if GenJnlLine6."Bill-to/Pay-to No." <> '' then
            exit(GenJnlLine6."Bill-to/Pay-to No.");

        GenJnlLine7.SetCurrentKey("Journal Template Name", "Journal Batch Name", "Posting Date", "Transaction No.");
        GenJnlLine7.SetRange("Journal Template Name", CurrGenJnlLine."Journal Template Name");
        GenJnlLine7.SetRange("Journal Batch Name", GenJnlLine6."Journal Batch Name");
        GenJnlLine7.SetRange("Posting Date", GenJnlLine6."Posting Date");
        GenJnlLine7.SetRange("Transaction No.", GenJnlLine6."Transaction No.");
        ...
        GenJnlLine7.SetRange("Document No.", CurrGenJnlLine."Document No.");
        ...
        if not GenJnlLine7.Find('-') or (GenJnlLine7.Next() <> 0) then
            Error(
              ...
              GenJnlLine6.FieldCaption(GenJnlLine6."Document No."),
              GenJnlLine6."Line No.");
    ...
    begin
        ...
        if TempGenJnlLine.Find('-') then
            repeat
                ...
                PostAllocations(GenJournalLine1, TempGenJnlLine, true);
            until TempGenJnlLine.Next() = 0;
    ...
    begin
        ...
        if (((GenJnlLine5."Document Type" = GenJnlLine5."Document Type"::Payment) or
            ...
            (GenJnlLine5."Applies-to Doc. Type" = GenJnlLine5."Applies-to Doc. Type"::Bill))
        then
        ...
        if ((GenJnlLine5."Document Type" = GenJnlLine5."Document Type"::Payment) and
            (CurrGenJnlLine."Account Type" = CurrGenJnlLine."Account Type"::Customer) and
            (GenJnlLine5."Applies-to Doc. Type" = GenJnlLine5."Applies-to Doc. Type"::Invoice))
        then begin
            CustLedgEntry2.SetCurrentKey("Document No.", "Document Type", "Customer No.");
        ...
        if (CurrGenJnlLine."Applies-to Doc. Type" = CurrGenJnlLine."Applies-to Doc. Type"::Bill) and
           (CurrGenJnlLine."Applies-to Doc. No." <> '')
        then begin
            GenJnlPostLine.SetIDBillSettlement(true);
            GenJnlPostLine.SetFromSettlement(false);
        end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnBeforeCheckCorrection(GenJournalLine: Record "Gen. Journal Line"; var IsHandled: Boolean; GenJnlTemplate: Record "Gen. Journal Template"; var LastDate: Date; var LastDocType: Enum "Gen. Journal Document Type"; var LastDocNo: Code[20]; var LastTempTransNo: Integer; var DocCorrection: Boolean)
    begin
    end;
