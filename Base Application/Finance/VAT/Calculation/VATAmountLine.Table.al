...
table 290 "VAT Amount Line"
{
    ...
    fields
    {
        ...
        field(10001; "Tax Area Code"; Code[20])
        {
    ...
    keys
    {
        key(Key1; "VAT Identifier", "VAT Calculation Type", "Tax Group Code", "Tax Area Code", "Use Tax", Positive)
        {
            Clustered = true;
        }
    }
...
#pragma warning restore AA0470
...
#if not CLEAN26
    [Obsolete('Replaced by procedures using Source Record.', '26.0')]
    procedure InsertNewLine(VATIdentifier: Code[20]; VATCalcType: Enum "Tax Calculation Type"; TaxGroupCode: Code[20]; TaxAreaCode: Code[20]; UseTax: Boolean; TaxRate: Decimal; IsPositive: Boolean; IsPrepayment: Boolean; NonDeductibleVATPct: Decimal)
    begin
        Rec.Init();
        Rec."VAT Identifier" := VATIdentifier;
        Rec."VAT Calculation Type" := VATCalcType;
    ...
    procedure GetAmtSubjectToSalesTax(): Decimal
    var
        SalesTaxBase: Decimal;
    begin
        if FindSet() then
            repeat
    ...
    procedure GetAmtExemptFromSalesTax(): Decimal
    var
        SalesTaxBase: Decimal;
    begin
        if FindSet() then
