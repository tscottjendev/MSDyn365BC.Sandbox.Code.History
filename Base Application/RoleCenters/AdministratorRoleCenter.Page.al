...
page 9018 "Administrator Role Center"
{
    ...
    actions
    {
        ...
        area(sections)
        {
            ...
            {
                Caption = 'Electronic Banking';
                action("Bank Directory")
                {
                    ApplicationArea = Basic, Suite;
                    Caption = 'Bank Directory';
                    RunObject = Page "Bank Directory";
                    ToolTip = 'View the bank addresses that are stored using the clearing number. When you install the DTA module, the current status of the domestic banks is imported into the table from an ASCII file.';
                }
                action("DTA Banks")
                {
                    ApplicationArea = Basic, Suite;
                    ...
                    ToolTip = 'View the list of banks that are set up for DatenTrägerAustausch (DTA) payments ';
                }
            }
            group("Analysis View")
            {
        ...
        area(processing)
        {
            ...
            group("&Report Selection")
            {
                ...
                action("Report Selection - VAT")
                {
                    ...
                    RunObject = Page "Report Selection - VAT Stmt.";
                    ToolTip = 'View the list of reports that are used to print VAT documents.';
                }
