...
codeunit 224 "EmplEntry-Apply Posted Entries"
{
    ...
    begin
        ...
        if DtldEmplLedgEntry.Find('-') then
            repeat
                if CheckVATSettlement(DtldEmplLedgEntry."Employee Ledger Entry No.", DtldEmplLedgEntry."Posting Date") then
                    Error(VATSettlementPostedErr, DtldEmplLedgEntry."Document Type", DtldEmplLedgEntry."Document No.");
    ...
    begin
        EmployeeLedgerEntry.Get(EmplLedgEntryNo);
        VATEntry.Reset();
    ...
    local procedure IsUnrealizedVAT(VATEntry: Record "VAT Entry"): Boolean
    var
