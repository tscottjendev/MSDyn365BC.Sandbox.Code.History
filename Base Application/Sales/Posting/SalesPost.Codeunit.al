...
#pragma warning restore AA0470
    ...
    begin
        ...
        if TransportMethod.Get(SalesHeader."Transport Method") and TransportMethod."Port/Airport" then
            SalesHeader.TestField("Exit Point", ErrorInfo.Create());
        ...
        if (SalesHeader.Ship or SalesHeader.Invoice) and not SalesHeader.IsCreditDocType() then begin
            SalesHeader.TestField("Payment Method Code", ErrorInfo.Create());
            SalesHeader.TestField("Payment Terms Code", ErrorInfo.Create());
        end;
        ...
        if ShouldCheckDueDate then begin
            ...
            PaymentTerms.VerifyMaxNoDaysTillDueDate(SalesHeader."Due Date", SalesHeader."Document Date", SalesHeader.FieldCaption("Due Date"));
        end;
        ...
        if SalesHeader."Currency Code" <> '' then
            ItemJnlLine2."Discount Amount" := Round(
                CurrExchRate.ExchangeAmtFCYToLCY(
                  ...
                  ItemChargeSalesLine."Quantity (Base)" * TempItemChargeAssgntSales."Qty. to Handle" / QtyToInvoice,
    ...
    begin
        ...
        if (SalesLineQty = 0) or (SalesLine."Unit Price" = 0) then begin
            ...
            if SalesLine."VAT Calculation Type" = SalesLine."VAT Calculation Type"::"Sales Tax" then begin
                ...
                SalesLine."EC %" := TempVATAmountLine."EC %";
            end;
                ...
                if SalesHeader."Prices Including VAT" then begin
                    if (TempVATAmountLine.CalcLineAmount() = 0) or (SalesLine."Line Amount" = 0) then begin
                        ...
                        TempVATAmountLineRemainder."VAT Amount" +=
                            ...
                            (SalesLine.CalcLineAmount() - SalesLine."Pmt. Discount Amount") /
                    ...
                    if SalesLine."VAT Calculation Type" = SalesLine."VAT Calculation Type"::"Full VAT" then begin
                        ...
                        SalesLine.Amount := SalesLine.CalcLineAmount() - SalesLine."Pmt. Discount Amount";
                        CalcVATBaseAmount(SalesHeader, SalesLine, TempVATAmountLine, TempVATAmountLineRemainder);
...
#pragma warning restore AA0005
    ...
    begin
        ...
        if SalesLineQty <> SalesLine.Quantity then begin
            SalesLine."Line Discount Amount" :=
                Round(SalesLine."Line Discount Amount" * SalesLineQty / SalesLine.Quantity, Currency."Amount Rounding Precision");
            SalesLine."Pmt. Discount Amount" :=
                Round(SalesLine."Pmt. Discount Amount" * SalesLineQty / SalesLine.Quantity, Currency."Amount Rounding Precision");
        end;
    end;
    ...
    begin
        ...
        SalesLine."Pmt. Discount Amount" := -SalesLine."Pmt. Discount Amount";
    ...
    begin
        ...
        Increment(TotalSalesLine."Pmt. Discount Amount", SalesLine."Pmt. Discount Amount");
