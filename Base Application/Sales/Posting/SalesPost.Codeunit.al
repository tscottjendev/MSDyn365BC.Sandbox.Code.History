...
#pragma warning restore AA0470
    ...
    begin
        ...
        SetTaxType(SalesHeader, TempSalesLineGlobal);
            ...
            if not SalesTaxCalculationOverridden then
                if SalesTaxCountry <> SalesTaxCountry::NoTax then begin
                    if UseExternalTaxEngine then
                        ...
                        SalesTaxCalculate.EndSalesTaxCalculation(SalesHeader2."Posting Date");
    ...
    begin
        ...
        if SalesHeader.Invoice then begin
            if SalesHeader."Prepmt. Include Tax" then
                PrepmtTaxRounding := SalesHeader."Prepmt. Sales Tax Rounding Amt"
            else
                PrepmtTaxRounding := 0;
            ...
            if SalesHeader."Prepmt. Include Tax" then
                SalesHeader."Prepmt. Sales Tax Rounding Amt" := PrepmtTaxRounding;
        end;
    ...
    local procedure PostInvoice(var SalesHeader: Record "Sales Header"; var CustLedgEntry: Record "Cust. Ledger Entry"; LineCount: Integer)
    var
        TotalAmount: Decimal;
        IsHandled: Boolean;
    begin
        ...
        if TaxOption = TaxOption::SalesTax then
            if SalesHeader."Tax Area Code" <> '' then begin
                ...
                SalesHeader."Sales Tax Amount Rounding" := SalesTaxCalculate.GetTotalTaxAmountRounding();
                if SalesHeader.Invoice then
                    TaxAmountDifference.ClearDocDifference(
                        Enum::"Sales Tax Document Area"::Sales.AsInteger(), SalesHeader."Document Type".AsInteger(), SalesHeader."No.");
            end;
        // Post customer entry
        if GuiAllowed() and not HideProgressWindow then
            Window.Update(4, 1);
    ...
    begin
        ...
        if SalesLine.Quantity = 0 then
            ...
            if GLSetup."VAT in Use" then
                ...
                if not (SalesLine.Type in [SalesLine.Type::"Fixed Asset", SalesLine.Type::"G/L Account"]) then
...
#pragma warning restore AL0432
    ...
    begin
        ...
        EInvoiceMgt.DeleteCFDITransportOperatorsAfterPosting(DATABASE::"Sales Header", SalesHeader."Document Type".AsInteger(), SalesHeader."No.");
    ...
    begin
                ...
                SalesHeader."Document Type"::"Credit Memo":
                    ...
                    if TempSalesLine.FindSet() then
                        repeat
                            ...
                            TempSalesLine.Modify();
                        until TempSalesLine.Next() = 0;
                    OnFinalizePostingOnAfterDecrementPrepmtAmtInvLCY(SalesHeader, TempDropShptPostBuffer);
        ...
        if SalesTaxCalculationOverridden then
            if SalesHeader."Document Type" in [SalesHeader."Document Type"::Order, SalesHeader."Document Type"::Invoice] then
                OnFinalizeInvoicePostingSalesTax(SalesHeader, SalesInvHeader, SuppressCommit, PreviewMode)
            else
                OnFinalizeCreditmemoPostingSalesTax(SalesHeader, SalesCrMemoHeader, SuppressCommit, PreviewMode);
        ...
        if SalesHeader.Invoice and (TaxOption = TaxOption::SalesTax) and UseExternalTaxEngine then
            if SalesHeader."Document Type" in [SalesHeader."Document Type"::Order, SalesHeader."Document Type"::Invoice] then
    ...
    begin
        ...
        GetGenPostingSetup(GenPostingSetup, SalesLine); // NA
        ...
        if InvoiceDiscountPosting then begin
            ...
            if not IsHandled then begin
                ...
                if (InvoicePostBuffer.Amount <> 0) or (InvoicePostBuffer."Amount (ACY)" <> 0) then begin
                    ...
                    ClearVATForSalesTax(InvoicePostBuffer);
    ...
    begin
        ...
        if ((SalesLineQty = 0) or (SalesLine."Unit Price" = 0)) and not
            SalesTaxCalculate.HasExciseTax(SalesLine."Tax Area Code", SalesLine."Tax Group Code", SalesLine."Tax Liable", SalesLine.Quantity, SalesHeader."Document Date") then begin
            ...
            if SalesLine."VAT Calculation Type" = SalesLine."VAT Calculation Type"::"Sales Tax" then begin
                ...
                then begin
                    ...
                    SalesLine."Amount Including VAT" := SalesLine.Amount;
                    OnDivideAmountOnAfterCalcSalesTaxGeneral(SalesHeader, SalesLine);
                end;
            end else begin
                IsHandled := false;
                OnDivideAmountOnBeforeTempVATAmountLineGet(SalesLine, TempVATAmountLine, IsHandled);
                if not IsHandled then
                    TempVATAmountLine.Get(SalesLine."VAT Identifier", SalesLine."VAT Calculation Type", SalesLine."Tax Group Code", SalesLine."Tax Area Code", false, SalesLine."Line Amount" >= 0);
...
#pragma warning restore AA0005
        ...
        OnAfterDivideAmount(SalesHeader, SalesLine, QtyType, SalesLineQty, TempVATAmountLine, TempVATAmountLineRemainder, TempSalesLineForSalesTax);
    end;
        ...
        if TempSalesLine.FindSet() then begin
            ...
            repeat
                ...
                if TempSalesLine."Prepmt Amt to Deduct" <> 0 then begin
                    ...
                    then
                        GenPostingSetup.Get(TempSalesLine."Gen. Bus. Posting Group", TempSalesLine."Gen. Prod. Posting Group");
                    ...
                    PrepmtAmtToDeductInclTax := (TempSalesLine."Prepmt Amt to Deduct" + PrepmtTaxRounding) * (1 + TempSalesLine."VAT %" / 100);
                    PrepmtAmtToDeductInclTaxRounded := TempSalesLine.CalcAmountIncludingTax(TempSalesLine."Prepmt Amt to Deduct" + PrepmtTaxRounding);
                    ...
                    if TempLineFound then begin
                        ...
                        if SalesHeader."Prepmt. Include Tax" then begin
                            ...
                            TempPrepmtSalesLine.Validate("Unit Price", TempPrepmtSalesLine."Unit Price" + TempSalesLine."Prepmt Amt to Deduct");
                        ...
                        PrepmtAmtToDeduct := InsertedPrepmtVATBaseToDeduct(SalesHeader, TempSalesLine, NextLineNo, 0, PrepmtAmtToDeductInclTaxRounded);
                        if SalesHeader."Prepmt. Include Tax" then
    ...
    procedure InsertedPrepmtVATBaseToDeduct(SalesHeader: Record "Sales Header"; SalesLine: Record "Sales Line"; PrepmtLineNo: Integer; TotalPrepmtAmtToDeduct: Decimal; PrepmtAmtToDeductInclTax: Decimal): Decimal
    var
    ...
    begin
        ...
        if SalesHeader."Prepmt. Include Tax" then
            TempPrepmtDeductLCYSalesLine."Prepmt Amt to Deduct" := PrepmtAmtToDeductInclTax;
    ...
    begin
        ...
        if PrepmtSalesLine.FindSet() then
            repeat
                ...
                UpdatePrepmtAmountInvBuf(PrepmtSalesLine."Line No.", ActualCurrencyFactor, SalesHeader."Prepmt. Include Tax");
            until PrepmtSalesLine.Next() = 0;
        PrepmtSalesLine.Reset();
    end;

    local procedure UpdatePrepmtAmountInvBuf(PrepmtSalesLineNo: Integer; CurrencyFactor: Decimal; PrepmtIncludeTax: Boolean)
    var
        PrepmtAmtRemainder: Decimal;
    begin
        TempPrepmtDeductLCYSalesLine.Reset();
        TempPrepmtDeductLCYSalesLine.SetRange(TempPrepmtDeductLCYSalesLine."Attached to Line No.", PrepmtSalesLineNo);
        if TempPrepmtDeductLCYSalesLine.FindSet(true) then
            repeat
                if PrepmtIncludeTax then
                    TempPrepmtDeductLCYSalesLine."Prepmt. Amount Inv. (LCY)" := TempPrepmtDeductLCYSalesLine."Prepmt Amt to Deduct"
                else
        ...
        if PrepmtSalesLine."Prepayment Line" then begin
            ...
            if TempPrepmtDeductLCYSalesLine.FindSet(true) then begin
                ...
                repeat
                    ...
                    if (not SalesHeader."Prepmt. Include Tax") and (SalesLine."Qty. to Invoice" <> TempPrepmtDeductLCYSalesLine."Qty. to Invoice") then
    ...
    begin
        ...
        TempICGenJnlLine.Validate(TempICGenJnlLine."Bal. VAT %");
    ...
    begin
        ...
        if (TempSalesLineForSalesTax."Prepayment %" = 100) and not TempSalesLineForSalesTax."Prepayment Line" and (TempSalesLineForSalesTax."Prepmt Amt to Deduct" <> 0) then
            TempSalesLineForSalesTax."Line Amount" := TempSalesLineForSalesTax."Prepmt Amt to Deduct" + TempSalesLineForSalesTax."Inv. Disc. Amount to Invoice"
        else
        ...
        OnAddSalesTaxLineToSalesTaxCalcOnBeforeTempSalesLineForSalesTaxInsert(SalesHeader, SalesLine, TempSalesLineForSalesTax);
    ...
    begin
        ...
        if TempSalesTaxAmtLine.Find('-') then begin
            repeat
                ...
                then begin
                    ...
                    GenJnlLine."VAT Reporting Date" := SalesHeader."VAT Reporting Date";
        ...
        OnAfterPostSalesTaxToGL(SalesHeader, GenJnlLineDocType.AsInteger(), GenJnlLineDocNo, GenJnlLineExtDocNo);
    end;

    local procedure PostSalesTaxToGLRounding(SalesHeader: Record "Sales Header"; LineAmount: Decimal; LineAmountLCY: Decimal)
    var
    ...
    begin
        ...
        OnBeforePostSalesTaxRoundingToGL(GenJournalLine, SalesHeader);
...
#if not CLEAN23
    local procedure ClearVATForSalesTax(var InvoicePostBuffer: Record "Invoice Post. Buffer")
    var
        IsHandled: Boolean;
    begin
        if TaxOption = TaxOption::SalesTax then begin
            IsHandled := false;
            OnClearVATForSalesTax(InvoicePostBuffer, IsHandled);
            if not IsHandled then
    ...
    begin
        ...
        EInvoiceMgt.InsertSalesCreditMemoCFDIRelations(SalesHeader, SalesCrMemoHeader."No.");
    ...
    begin
        if GLSetup."VAT in Use" then
            ...
            if (SalesLine.Type.AsInteger() >= SalesLine.Type::"G/L Account".AsInteger()) and
                ...
                repeat
                    if SalesLine."VAT Calculation Type" = SalesLine."VAT Calculation Type"::"Sales Tax" then begin
                        ...
                        if TaxOption = 0 then
                            ...
                            if TaxOption = TaxOption::SalesTax then
                                Error(EveryLineMustHaveSameErr, SalesLine.FieldCaption("VAT Calculation Type"), TaxOption);
                    OnSetTaxTypeOnAfterAddSalesTaxLineToSalesTaxCalc(SalesLine);
                until SalesLine.Next() = 0;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnAfterPostSalesTaxToGL(var SalesHeader: Record "Sales Header"; GenJnlLineDocType: Integer; GenJnlLineDocNo: Code[20]; GenJnlLineExtDocNo: Code[35])
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnAfterDivideAmount(var SalesHeader: Record "Sales Header"; var SalesLine: Record "Sales Line"; QtyType: Option General,Invoicing,Shipping; SalesLineQty: Decimal; var TempVATAmountLine: Record "VAT Amount Line" temporary; var TempVATAmountLineRemainder: Record "VAT Amount Line" temporary; var TempSalesLineForSalesTax: Record "Sales Line" temporary)
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnBeforePostUpdateOrderLine(SalesHeader: Record "Sales Header"; var SalesLine: Record "Sales Line" temporary; CommitIsSuppressed: Boolean; var SalesSetup: Record "Sales & Receivables Setup")
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnBeforeCalculateSalesTax(var SalesHeader: Record "Sales Header"; var SalesLine: Record "Sales Line"; var SalesTaxAmountLine: Record "Sales Tax Amount Line"; var LocalwasProcessed: Boolean; CommitIsSuppressed: Boolean)
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnFinalizeInvoicePostingSalesTax(var SalesHeader: Record "Sales Header"; var SalesInvoiceHeader: Record "Sales Invoice Header"; CommitIsSuppressed: Boolean; PreviewMode: Boolean)
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnFinalizeCreditmemoPostingSalesTax(var SalesHeader: Record "Sales Header"; var SalesCrMemoHeader: Record "Sales Cr.Memo Header"; CommitIsSuppressed: Boolean; PreviewMode: Boolean)
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnBeforePostSalesTaxRoundingToGL(var GenJnlLine: Record "Gen. Journal Line"; SalesHeader: Record "Sales Header")
    begin
    end;
...
#pragma warning restore AS0072
    ...
    [IntegrationEvent(false, false)]
    local procedure OnDivideAmountOnAfterCalcSalesTaxGeneral(SalesHeader: Record "Sales Header"; var SalesLine: Record "Sales Line")
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnBeforePostSalesTaxToGeneralLedger(var SalesHeader: Record "Sales Header"; var LineCount: Integer; var TempSalesTaxAmountLine: Record "Sales Tax Amount Line" temporary; var Window: Dialog; var SalesTaxCountry: Option US,CA,,,,,,,,,,,,NoTax; var GenJnlLineDocNo: Code[20]; var GenJnlLineExtDocNo: Code[35]; var GenJnlLineDocType: Enum "Gen. Journal Document Type"; var SourceCode: Code[10]; var Currency: Record Currency; var GenJnlPostLine: Codeunit "Gen. Jnl.-Post Line"; var TotalSalesLineLCY: Record "Sales Line"; var TotalSalesLine: Record "Sales Line"; var TempSalesLineGlobal: Record "Sales Line" temporary; var IsHandled: Boolean)
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnFinalizePostingOnAfterDecrementPrepmtAmtInvLCY(var SalesHeader: Record "Sales Header"; var TempDropShptPostBuffer: Record "Drop Shpt. Post. Buffer" temporary);
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnAddSalesTaxLineToSalesTaxCalcOnBeforeTempSalesLineForSalesTaxInsert(var SalesHeader: Record "Sales Header"; var SalesLine: Record "Sales Line"; var TempSalesLineForSalesTax: Record "Sales Line" temporary)
    begin
    end;
