...
#pragma warning restore AA0470
...
#endif
        Text1130003Err: Label 'Please enter at least one Payment Line for %1 %2.';
        SpecifyInstallmentErr: Label 'To specify the installment to apply to please select the %1 or use the function Apply Entries.', Comment = '%1 = Applies-to Occurrence No.';
    ...
    begin
        ...
        if SalesHeader.Invoice and (SalesHeader."Operation Occurred Date" < GLSetup."Last Gen. Jour. Printing Date") then
            SalesHeader.FieldError(SalesHeader."Operation Occurred Date", StrSubstNo(Text1130007Err, GLSetup."Last Gen. Jour. Printing Date"));
        ...
        if SalesHeader.Invoice then
            InsertPostedPayment(SalesHeader);
    ...
    begin
        ...
        if SalesHeader.Invoice then
            ValidateIncludeInVATReport(SalesHeader);
    ...
    begin
        ...
        if SalesHeader."Document Type" in
           [SalesHeader."Document Type"::Order, SalesHeader."Document Type"::Invoice, SalesHeader."Document Type"::"Credit Memo"]
        then
            CheckTariff(SalesHeader, SalesLine);
    ...
    begin
        ...
        if PaymentMethod.Get(SalesHeader."Payment Method Code") then
            if PaymentMethod."Free Type" <> PaymentMethod."Free Type"::" " then
                PostPayment(SalesHeader, PaymentMethod);
    ...
    begin
        ...
        SalesLine.TestField("VAT Identifier");
    end;
...
#pragma warning restore AL0432
    ...
    begin
        ...
        if SalesHeader.Invoice and (SalesHeader."Posting No." = '') then begin
            ...
            then
                if not PreviewMode then begin
                    SalesHeader."Posting No." := NoSeriesCodeunit.GetNextNo(SalesHeader."Posting No. Series", SalesHeader."Posting Date");
                    if NoSeriesCodeunit.IsNoSeriesInDateOrder(SalesHeader."Posting No. Series") then
                        DateOrderSeriesUsed := true;
                    ModifyHeader := true;
                end;
...
#pragma warning restore AA0005
        ...
        if TempSalesLine.FindSet() then begin
            ...
            repeat
                ...
                if TempSalesLine."Prepmt Amt to Deduct" <> 0 then begin
                    ...
                    if not IsHandled then
                        GLAcc.Get(VATPostingSetup."Sales Prepayments Account");
    ...
    begin
        ...
        GenJnlLine."Applies-to Occurrence No." := SalesHeader."Applies-to Occurrence No.";
        GenJnlLine.CopyFromSalesHeaderPayment(SalesHeader);
        if PaymentMethod.Get(SalesHeader."Payment Method Code") and Bill.Get(PaymentMethod."Bill Code") then begin
            GenJnlLine."Bank Receipt" := Bill."Bank Receipt";
            GenJnlLine."Allow Issue" := Bill."Allow Issue";
        end;
    ...
    local procedure PostPayment(SalesHeader: Record "Sales Header"; PaymentMethod: Record "Payment Method")
    var
        CustPostingGroup: Record "Customer Posting Group";
        GenJnlLine: Record "Gen. Journal Line";
    begin
        CustPostingGroup.Get(SalesHeader."Customer Posting Group");
        CustPostingGroup.TestField("Free Invoice Account");
        GenJnlLine.Init();
        ...
        GenJnlLine."Bal. Account No." := CustPostingGroup."Free Invoice Account";
        GenJnlLine."Currency Code" := SalesHeader."Currency Code";
        if PaymentMethod."Free Type" = PaymentMethod."Free Type"::"Total Amt." then begin
            GenJnlLine.Amount := TotalSalesLine."Amount Including VAT";
            GenJnlLine."Amount (LCY)" := TotalSalesLineLCY."Amount Including VAT";
        end else
            if PaymentMethod."Free Type" = PaymentMethod."Free Type"::"Only VAT Amt." then begin
                GenJnlLine.Amount := TotalSalesLine.Amount;
                GenJnlLine."Amount (LCY)" := TotalSalesLineLCY.Amount;
            end;
        ...
        OnBeforePostPayment(GenJnlLine, SalesHeader, SuppressCommit, PreviewMode, GenJnlPostLine);
        GenJnlPostLine.RunWithCheck(GenJnlLine);
        OnAfterPostPayment(GenJnlLine, SalesHeader, SuppressCommit, PreviewMode, GenJnlPostLine);
    end;
    ...
    begin
        ...
        SetAmountsForBalancingEntry(CustLedgEntry, TotalSalesLine2, TotalSalesLineLCY2, GenJnlLine, SalesHeader, TotalRemainPmtDiscPossible);
    ...
    local procedure SetAmountsForBalancingEntry(CustLedgEntry: Record "Cust. Ledger Entry"; TotalSalesLine2: Record "Sales Line"; TotalSalesLineLCY2: Record "Sales Line"; var GenJnlLine: Record "Gen. Journal Line"; SalesHeader: Record "Sales Header"; TotalRemainPmtDiscPossible: Decimal)
    var
        IsHandled: Boolean;
    begin
        IsHandled := false;
        ...
        if PaymentMethod."Free Type" = PaymentMethod."Free Type"::"Only VAT Amt." then begin
            GenJnlLine.Amount := TotalSalesLine2."Amount Including VAT" - TotalSalesLine2.Amount;
            GenJnlLine."Amount (LCY)" := TotalSalesLineLCY2."Amount Including VAT" - TotalSalesLineLCY2.Amount;
        end else begin
    ...
    local procedure ValidateIncludeInVATReport(SalesHeaderParam: Record "Sales Header")
    var
    ...
    begin
        ...
        InclInVATReportValidation.ValidateSalesHeader(SalesHeaderParam, InclInVATReportErrorLogTemp);
        OnValidateIncludeInVATReportAfterValidate(SalesHeaderParam, InclInVATReportErrorLogTemp);
        if InclInVATReportErrorLogTemp.FindFirst() then begin
            if InclInVATReportErrorLogTemp."Line No." <> 0 then
                Error(
                  InclInVATReportErrorLogTemp."Error Message" + ' ' +
                  StrSubstNo(Text1130009Err, InclInVATReportErrorLogTemp."Line No."));

            Error(InclInVATReportErrorLogTemp."Error Message")
        end;
    end;
    ...
    local procedure CheckTariff(SalesHeader: Record "Sales Header"; SalesLine: Record "Sales Line")
    var
        VATPostingSetup: Record "VAT Posting Setup";
    begin
        if VATPostingSetup.IsEUService(SalesLine."VAT Bus. Posting Group", SalesLine."VAT Prod. Posting Group") then begin
            if VATPostingSetup."Service Tariff No. Mandatory" then begin
    ...
    local procedure CheckForIT(SalesHeader: Record "Sales Header")
    var
        VATExemption: Record "VAT Exemption";
        Check: Boolean;
    begin
        if PaymentMethod.Get(SalesHeader."Payment Method Code") then
            if PaymentMethod."Free Type" = PaymentMethod."Free Type"::"Total Amt." then
                PaymentMethod.TestField("Bal. Account No.", '', ErrorInfo.Create());
        if not SalesHeader.FindVATExemption(VATExemption, Check, true) then
            if Check then
                Error(ErrorInfo.Create(Text1130008Err, true, SalesHeader));
        if SalesHeader."Applies-to Doc. No." <> '' then
            if SalesHeader."Applies-to Occurrence No." = 0 then
                Error(ErrorInfo.Create(StrSubstNo(SpecifyInstallmentErr, SalesHeader.FieldCaption("Applies-to Occurrence No.")), true, SalesHeader));
    end;
    ...
    local procedure InsertPostedPayment(SalesHeader: Record "Sales Header")
    var
    ...
    begin
        ...
        PaymentTermsLine.SetRange("Sales/Purchase", PaymentTermsLine."Sales/Purchase"::Sales);
        PaymentTermsLine.SetRange(Type, SalesHeader."Document Type");
        PaymentTermsLine.SetRange(Code, SalesHeader."No.");
        if PaymentTermsLine.Find('-') then
            repeat
                PaymentTermsLine.TestField("Due Date");
                PostedPayments.Init();
                PostedPayments."Sales/Purchase" := PaymentTermsLine."Sales/Purchase";
                if SalesHeader."Document Type" in [SalesHeader."Document Type"::Order, SalesHeader."Document Type"::Invoice] then begin
                    PostedPayments.Type := PostedPayments.Type::Invoice;
                    PostedPayments.Code := SalesInvHeader."No.";
                end else begin
                    PostedPayments.Type := PostedPayments.Type::"Credit Memo";
                    PostedPayments.Code := SalesCrMemoHeader."No.";
                end;
                PostedPayments."Line No." := PaymentTermsLine."Line No.";
                PostedPayments."Payment %" := PaymentTermsLine."Payment %";
                PostedPayments."Due Date Calculation" := PaymentTermsLine."Due Date Calculation";
                PostedPayments."Discount Date Calculation" := PaymentTermsLine."Discount Date Calculation";
                PostedPayments."Discount %" := PaymentTermsLine."Discount %";
                PostedPayments."Due Date" := PaymentTermsLine."Due Date";
                PostedPayments."Pmt. Discount Date" := PaymentTermsLine."Pmt. Discount Date";
                OnInsertPostedPaymentOnBeforeInsertPostedPayments(SalesHeader, PaymentTermsLine, PostedPayments);
                PostedPayments.Insert();
                PaymentJournalLine.Init();
                PaymentJournalLine."Sales/Purchase" := 0;
                PaymentJournalLine.Type := PaymentJournalLine.Type::"General Journal";
                PaymentJournalLine."Line No." := PaymentTermsLine."Line No.";
                PaymentJournalLine."Payment %" := PaymentTermsLine."Payment %";
                PaymentJournalLine."Due Date Calculation" := PaymentTermsLine."Due Date Calculation";
                PaymentJournalLine."Discount Date Calculation" := PaymentTermsLine."Discount Date Calculation";
                PaymentJournalLine."Discount %" := PaymentTermsLine."Discount %";
                PaymentJournalLine."Due Date" := PaymentTermsLine."Due Date";
                PaymentJournalLine."Pmt. Discount Date" := PaymentTermsLine."Pmt. Discount Date";
                OnInsertPostedPaymentOnBeforeInsertPaymentJournalLine(SalesHeader, PaymentTermsLine, PaymentJournalLine);
                PaymentJournalLine.Insert();
            until PaymentTermsLine.Next() = 0
        else
            if SalesHeader."Payment Terms Code" <> '' then
                Error(Text1130003Err, SalesHeader.FieldCaption("Payment Terms Code"), SalesHeader."Payment Terms Code");
    end;
    ...
    begin
        ...
        GenJnlLine.Validate(GenJnlLine."Document Date");
        GenJnlLine."Operation Occurred Date" := SalesHeader."Operation Occurred Date";
        ...
        GenJnlLine."First Name" := SalesHeader."First Name";
        GenJnlLine."Last Name" := SalesHeader."Last Name";
        GenJnlLine."Date of Birth" := SalesHeader."Date of Birth";
        GenJnlLine."Place of Birth" := SalesHeader."Place of Birth";
        GenJnlLine."Tax Representative Type" :=
          GenJnlLine.ConvertSalesTaxRepresentativeTypeToGenJnlLine(SalesHeader."Tax Representative Type");
        GenJnlLine."Tax Representative No." := SalesHeader."Tax Representative No.";
        GenJnlLine."Payment Method Code" := SalesHeader."Payment Method Code";
        ...
        GenJnlLine."Deductible %" := InvoicePostBuffer."Deductible %";
        GenJnlLine."VAT Identifier" := InvoicePostBuffer."VAT Identifier";
        GenJnlLine."Refers to Period" := InvoicePostBuffer."Refers to Period";
        GenJnlLine."Contract No." := InvoicePostBuffer."Contract No.";
        GenJnlLine."Include in VAT Transac. Rep." := InvoicePostBuffer."Include in VAT Transac. Rep.";
        GenJnlLine."Service Tariff No." := InvoicePostBuffer."Service Tariff No.";
        GenJnlLine."Transport Method" := InvoicePostBuffer."Transport Method";
    ...
    begin
        ...
        SalesHeader.TestField("Operation Occurred Date", ErrorInfo.Create());
        SalesHeader.TestField("Operation Type", ErrorInfo.Create());
    ...
    [IntegrationEvent(false, false)]
    local procedure OnAfterPostPayment(var GenJnlLine: Record "Gen. Journal Line"; SalesHeader: Record "Sales Header"; SuppressCommit: Boolean; PreviewMode: Boolean; var GenJnlPostLine: Codeunit "Gen. Jnl.-Post Line");
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnBeforePostPayment(var GenJnlLine: Record "Gen. Journal Line"; SalesHeader: Record "Sales Header"; SuppressCommit: Boolean; PreviewMode: Boolean; var GenJnlPostLine: Codeunit "Gen. Jnl.-Post Line");
    begin
    end;
