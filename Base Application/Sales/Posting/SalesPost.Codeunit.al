...
#pragma warning restore AA0470
        ...
        QuoteManagement: Codeunit QuoteMgt;
...
#pragma warning restore AA0470
    ...
    begin
        ...
        if SalesHeader.Invoice and SalesSetup."Apply Inv. Round. Amt. To VAT" then
            UpdateVATDiff(SalesHeader, TempSalesLineGlobal, TempVATAmountLine);
        ...
        if CheckQuoteMgtPermission() then begin
            if SalesHeader.Invoice and
               (SalesHeader."Document Type" in [SalesHeader."Document Type"::Order, SalesHeader."Document Type"::Invoice])
            then
                QuoteManagement.RecalcPostedInvoice(SalesInvHeader);
            if SalesHeader.Invoice and (SalesHeader."Document Type" = SalesHeader."Document Type"::"Credit Memo") then
                QuoteManagement.RecalcPostedCreditMemo(SalesCrMemoHeader);
        end;
    ...
    begin
        ...
        UpdatePostingDescriptionOnInvoice(SalesHeader."Posting No.", SalesHeader, ModifyHeader);
...
#pragma warning restore AA0005
    ...
    begin
        ...
        if OldSalesLine.FindSet() then
            repeat
                ...
                if not IsHandled then
                    if SalesLine."Quote Variant" <> SalesLine."Quote Variant"::Variant then
                        DivideAmount(SalesHeader, SalesLine, QtyType, SalesLineQty, TempVATAmountLine, TempVATAmountLineRemainder, IncludePrepayments)
                    else
                        DivideAmount(SalesHeader, SalesLine, QtyType, 0, TempVATAmountLine, TempVATAmountLineRemainder, IncludePrepayments);
                ...
                if InsertSalesLine then begin
                    ...
                    if SalesLine."Quote Variant" = SalesLine."Quote Variant"::Variant then begin
                        NewSalesLine."Unit Cost (LCY)" := 0;
                        NewSalesLine.Quantity := 0;
                        NewSalesLine."Units per Parcel" := 0;
                        NewSalesLine."Net Weight" := 0;
                        NewSalesLine."Gross Weight" := 0;
                        NewSalesLine."Unit Volume" := 0;
                    end;
                    IsHandled := false;
            ...
            repeat
                ...
                if TempItemChargeAssgntSales.FindSet() then
                    repeat
                        ...
                        ChargedItem.Get(TempItemChargeAssgntSales."Item No.");
                    until TempItemChargeAssgntSales.Next() = 0;
    ...
    [Scope('OnPrem')]
    procedure UpdateVATDiff(SalesHeader: Record "Sales Header"; var TempCombinedSalesLine: Record "Sales Line" temporary; var TempVATAmountLine: Record "VAT Amount Line" temporary)
    var
        TmpSalesLine2: Record "Sales Line" temporary;
        QtyToInvCorrected: Boolean;
    begin
        if SalesHeader."Document Type" in
           [SalesHeader."Document Type"::Order, SalesHeader."Document Type"::"Return Order"]
        then begin
            TempCombinedSalesLine.Reset();
            if TempCombinedSalesLine.FindSet() then
                repeat
                    if TempCombinedSalesLine."Prepayment Line" then begin
                        TmpSalesLine2 := TempCombinedSalesLine;
                        TmpSalesLine2.Insert();
                    end else
                        if TempCombinedSalesLine."Qty. to Invoice" <> 0 then begin
                            TmpSalesLine2 := TempCombinedSalesLine;
                            TmpSalesLine2.Insert();
                            if not SalesHeader.Ship then begin
                                TmpSalesLine2."Qty. to Ship" := 0;
                                TmpSalesLine2."Qty. to Ship (Base)" := 0;
                            end;
                            if not SalesHeader.Receive then begin
                                TmpSalesLine2."Return Qty. to Receive" := 0;
                                TmpSalesLine2."Return Qty. to Receive (Base)" := 0;
                            end;
                            if Abs(TmpSalesLine2."Qty. to Invoice") > Abs(TmpSalesLine2.MaxQtyToInvoice()) then
                                TmpSalesLine2.InitQtyToInvoice();
                            if TmpSalesLine2."Qty. to Invoice" <> TempCombinedSalesLine."Qty. to Invoice" then begin
                                TmpSalesLine2.Modify();
                                QtyToInvCorrected := true;
                            end;
                        end;
                until TempCombinedSalesLine.Next() = 0;
    ...
    local procedure UpdatePostingDescriptionOnInvoice(PostingNo: Code[20]; var SalesHeader: Record "Sales Header"; var ModifyHeader: Boolean)
    var
        IsHandled: Boolean;
    begin
        IsHandled := false;
        OnBeforeUpdatePostingDescriptionOnInvoice(SalesHeader, PostingNo, ModifyHeader, IsHandled);
        if IsHandled then
            exit;

        if (SalesHeader."Posting Description" = Format(SalesHeader."Document Type") + ' ' + SalesHeader."No.") or (PostingNo <> '') then begin
            if not (SalesHeader."Document Type" in [SalesHeader."Document Type"::Order, SalesHeader."Document Type"::"Return Order"]) then
                SalesHeader."Posting Description" :=
                  CopyStr(CopyStr(Format(SalesHeader."Document Type"), 1, 4) + '. ' + SalesHeader."Posting No." + '/' +
                    SalesHeader."Bill-to Name", 1, MaxStrLen(SalesHeader."Posting Description"))
            else
                SalesHeader."Posting Description" :=
                  CopyStr(CopyStr(Format(SalesHeader."Document Type"), 1, 4) + '. ' + SalesHeader."No." + '/' +
                    SalesHeader."Bill-to Name", 1, MaxStrLen(SalesHeader."Posting Description"));
            ModifyHeader := true;
        end;
    end;
