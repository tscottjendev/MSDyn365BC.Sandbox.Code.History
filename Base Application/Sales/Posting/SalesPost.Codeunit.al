        ...
        if (SalesLineQty = 0) or (SalesLine."Unit Price" = 0) then begin
            ...
            if not IsHandled then
                if SalesHeader."Prices Including VAT" then begin
                    ...
                    if (SalesLine."VAT %" <> 0) and (TempVATAmountLine.CalcLineAmount() <> 0) then begin
                        TempVATAmountLineRemainder."VAT Base (Lowered)" +=
                            TempVATAmountLine."VAT Base" * (1 - SalesHeader."VAT Base Discount %" / 100) *
                            SalesLine.CalcLineAmount() / TempVATAmountLine.CalcLineAmount();
                        SalesLine."VAT Base Amount" :=
                            Round(TempVATAmountLineRemainder."VAT Base (Lowered)", Currency."Amount Rounding Precision");
                        TempVATAmountLineRemainder."VAT Base (Lowered)" :=
                            TempVATAmountLineRemainder."VAT Base (Lowered)" - SalesLine."VAT Base Amount";
                    end else
            ...
            if TempVATAmountLine.CalcLineAmount() <> 0 then begin
                ...
                TempVATAmountLineRemainder."Pmt. Discount Amount" :=
                  TempVATAmountLineRemainder."Pmt. Discount Amount" - SalesLine."Pmt. Discount Amount";
            end;
