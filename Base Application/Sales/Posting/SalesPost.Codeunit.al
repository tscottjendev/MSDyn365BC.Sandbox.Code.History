...
#pragma warning restore AA0005
    ...
    begin
        ...
        if SalesHeader.Invoice then
            InsertInvoicedShipment(
              SalesInvHeader."No.", SalesLine."Line No.", SalesShptLine."Document No.", SalesShptLine."Line No.",
              SalesShptLine."Quantity Invoiced", SalesShptLine.Quantity, SalesInvHeader."Posting Date");
    ...
    local procedure InsertInvoicedShipment(InvoiceNo: Code[20]; InvoiceLineNo: Integer; ShipmentNo: Code[20]; ShipmentLineNo: Integer; QtyToInvoice: Decimal; QtyToShip: Decimal; PostingDate: Date)
    var
        ShipmentInvoiced: Record "Shipment Invoiced";
        IsHandled: Boolean;
    begin
        IsHandled := false;
        OnBeforeInsertInvoicedShipment(ShipmentInvoiced, IsHandled);
        if IsHandled then
            exit;

        ShipmentInvoiced.Init();
        ShipmentInvoiced."Invoice No." := InvoiceNo;
        ShipmentInvoiced."Invoice Line No." := InvoiceLineNo;
        ShipmentInvoiced."Shipment No." := ShipmentNo;
        ShipmentInvoiced."Shipment Line No." := ShipmentLineNo;
        ShipmentInvoiced."Qty. to Invoice" := QtyToInvoice;
        ShipmentInvoiced."Qty. to Ship" := QtyToShip;
        ShipmentInvoiced."Posting Date" := PostingDate;
        ShipmentInvoiced."User ID" := UserId;
        if ShipmentInvoiced.Insert() then;
    end;
    ...
    begin
        ...
        if SalesShptLine.FindFirst() then begin
            ...
            repeat
                ...
                InsertInvoicedShipment(
                  SalesInvHeader."No.", SalesLine."Line No.", SalesShptLine."Document No.", SalesShptLine."Line No.",
                  -QtyToBeInvoiced, SalesShptLine.Quantity, SalesInvHeader."Posting Date");
    ...
    [IntegrationEvent(false, false)]
    local procedure OnBeforeInsertInvoicedShipment(var ShipmentInvoiced: Record "Shipment Invoiced"; var IsHandled: Boolean)
    begin
    end;
