...
#pragma warning restore AA0100
    ...
    var
        ...
        WorkDateErr: Label 'When using workdate for applying/unapplying, the workdate must not be before the latest posting date %1 on the entries to apply.';
            ...
            if TempApplyingCustLedgEntry."Entry No." <> 0 then begin
                ...
                if GLSetup."Use Workdate for Appl./Unappl." then begin
                    if ApplicationDate > WorkDate() then
                        Error(WorkDateErr, ApplicationDate);
