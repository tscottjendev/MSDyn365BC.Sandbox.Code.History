...
#pragma warning restore AS0086
...
#endif
        field(12100; "Fiscal Code"; Code[20])
        {
            Caption = 'Fiscal Code';
        ...
        field(12101; "Register Company No."; Text[50])
        {
            Caption = 'Register Company No.';
        }
        field(12104; "REA No."; Code[10])
        {
            Caption = 'REA No.';
        }
        field(12108; "FD Address"; Text[50])
        {
            Caption = 'FD Address';
        }
        field(12109; "FD Post Code"; Code[20])
        {
            Caption = 'FD Post Code';
            TableRelation = if ("Country/Region Code" = const('')) "Post Code".Code
            else
            if ("Country/Region Code" = filter(<> '')) "Post Code".Code where("Country/Region Code" = field("Country/Region Code"));
            ValidateTableRelation = false;
            ...
            var
                CountryCode: Code[10];
            begin
                PostCode.ValidatePostCode("FD City", "FD Post Code", "FD County", CountryCode, (CurrFieldNo <> 0) and GuiAllowed);
            end;
        }
        field(12110; "FD City"; Text[50])
        {
            Caption = 'FD City';
            TableRelation = if ("Country/Region Code" = const('')) "Post Code".City
            else
            ...
            var
                CountryCode: Code[10];
            begin
                PostCode.ValidateCity("FD City", "FD Post Code", "FD County", CountryCode, (CurrFieldNo <> 0) and GuiAllowed);
            end;
        }
        field(12111; "FD County"; Text[50])
        {
            Caption = 'FD County';
        }
        field(12112; "SIA Code"; Code[5])
        {
            Caption = 'SIA Code';
        }
        field(12113; "Authority County"; Code[20])
        {
            Caption = 'Authority County';
        }
        field(12114; "Autoriz. No."; Code[10])
        {
            Caption = 'Autoriz. No.';
        }
        field(12115; "Autoriz. Date"; Date)
        {
            Caption = 'Autoriz. Date';
        }
        field(12116; "Signature on Bill"; Text[20])
        {
            Caption = 'Signature on Bill';
        }
        field(12118; BBAN; Code[30])
        {
            Caption = 'BBAN';
        }
        field(12119; "Tax Representative No."; Code[20])
        {
        ...
        field(12122; "Accounting Manager No."; Code[20])
        {
            Caption = 'Accounting Manager No.';
            TableRelation = "Company Officials";
        }
        field(12123; "Finance Manager No."; Code[20])
        {
        ...
        field(12126; "Transmission Intermediary No."; Code[20])
        {
            Caption = 'Transmission Intermediary No.';
            TableRelation = Vendor;
        }
        field(12127; "Shareholder Status"; Option)
        {
            Caption = 'Shareholder Status';
            OptionCaption = ' ,One Shareholder,Multiple Shareholders';
            OptionMembers = " ","One Shareholder","Multiple Shareholders";
        }
        field(12128; "Liquidation Status"; Option)
        {
            Caption = 'Liquidation Status';
            OptionCaption = ' ,Not in Liquidation,In Liquidation';
            OptionMembers = " ","Not in Liquidation","In Liquidation";
        }
        field(12129; "Registry Office Province"; Code[2])
        {
            Caption = 'Registry Office Province';
        }
...
#pragma warning restore AA0470
    ...
    begin
        ...
        if not CheckITIBAN(IBANCode) then
            exit;
        Modulus97 := 97;
        if (StrLen(IBANCode) <= 5) or (StrLen(IBANCode) > 34) then begin
    ...
    [Scope('OnPrem')]
    procedure CheckITIBAN(IBANCode: Code[100]): Boolean
    var
        ABICABCode: Record "ABI/CAB Codes";
        OriginalIBANCode: Code[100];
    begin
        OriginalIBANCode := IBANCode;
        if not (CopyStr(IBANCode, 1, 2) in ['IT', 'SM']) then
            exit(true);
        if DelChr(CopyStr(IBANCode, 3, 2), '=', '0123456789') <> '' then begin
            IBANError(OriginalIBANCode);
            exit(false);
        end;
        if StrLen(IBANCode) <> 27 then begin
            IBANError(OriginalIBANCode);
            exit(false);
        end;
        if CopyStr(IBANCode, 5, 1) <> ABICABCode.CalcCIN(CopyStr(IBANCode, 6, 5), CopyStr(IBANCode, 11, 5), CopyStr(IBANCode, 16, 12)) then begin
            IBANError(OriginalIBANCode);
            exit(false);
        end;
        exit(true);
    ...
    procedure GetTDDAddr(var AddrArray: array[8] of Text[100])
    var
    ...
    local procedure ValidateBankAccountNo()
    var
    ...
    begin
        if CopyStr(DelChr(IBAN), 1, 2) in ['IT', 'SM'] then begin
            ...
            if (ABI <> '') and (CAB <> '') and ("Bank Account No." <> '') then
                AbiCabCodes.CalcBBAN(ABI, CAB, "Bank Account No.");
        end;
    end;
