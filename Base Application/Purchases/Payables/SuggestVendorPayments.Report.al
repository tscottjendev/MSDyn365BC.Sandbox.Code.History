            ...
            area(content)
            {
                group(Options)
                {
                    ...
                    group("Find Payments")
                    {
                        ...
                        field(ExcludeCreditMemos; ExcludeCreditMemos)
                        {
                            ApplicationArea = Basic, Suite;
                            Caption = 'Exclude Credit Memos';
                            ToolTip = 'Specifies whether to exclude payment of credit memos from the suggested payments.';
                        }
                    }
...
#pragma warning restore AA0470
    ...
    begin
        InitializeRequest(
          ...
          Enum::"Bank Payment Type".FromInteger(BankPmtType));
    ...
    begin
        ...
        if SummarizePerVend then
            SetTempPaymentBufferSummarized(TempPayableVendorLedgerEntry2, TempPayableVendorLedgerEntry);
                    ...
                    if HandledEntry then begin
                        ...
                        if IsNotAppliedEntry(GenJnlLine, VendLedgEntry) then begin
                            ...
                            if SummarizePerVend then begin
                                TempVendorPaymentBuffer."Vendor Ledg. Entry No." := GetVendorEntryNoSummarized(TempPayableVendorLedgerEntry2, TempPayableVendorLedgerEntry);
    ...
    local procedure SetTempPaymentBufferSummarized(var TempPayableVendorLedgerEntrySum: Record "Payable Vendor Ledger Entry" temporary; var TempPayableVendorLedgerEntry: Record "Payable Vendor Ledger Entry" temporary)
    var
        GenJournalLine: Record "Gen. Journal Line";
        VendorLedgerEntry: Record "Vendor Ledger Entry";
    begin
        if TempPayableVendorLedgerEntry.FindSet() then
            repeat
                VendorLedgerEntry.Get(TempPayableVendorLedgerEntry."Vendor Ledg. Entry No.");
                SetPostingDate(GenJournalLine, VendorLedgerEntry."Due Date", PostingDate);
                if VendorLedgerEntry."Posting Date" <= GenJournalLine."Posting Date" then begin
                    FilterPayableVendorEntriesSummarized(TempPayableVendorLedgerEntrySum, TempPayableVendorLedgerEntry);
                    if TempPayableVendorLedgerEntrySum.FindFirst() then begin
                        TempPayableVendorLedgerEntrySum."Vendor Ledg. Entry No." := 0;
                        TempPayableVendorLedgerEntrySum.Modify();
                    end else begin
                        TempPayableVendorLedgerEntrySum.TransferFields(TempPayableVendorLedgerEntry);
                        TempPayableVendorLedgerEntrySum.Insert();
                    end;
                end;
            until TempPayableVendorLedgerEntry.Next() = 0;
    end;

    local procedure GetVendorEntryNoSummarized(var TempPayableVendorLedgerEntrySum: Record "Payable Vendor Ledger Entry" temporary; TempPayableVendorLedgerEntry: Record "Payable Vendor Ledger Entry" temporary): Integer
    begin
        FilterPayableVendorEntriesSummarized(TempPayableVendorLedgerEntrySum, TempPayableVendorLedgerEntry);
        if TempPayableVendorLedgerEntrySum.FindFirst() then
            exit(TempPayableVendorLedgerEntrySum."Vendor Ledg. Entry No.");
        exit(0);
    end;

    local procedure FilterPayableVendorEntriesSummarized(var TempPayableVendorLedgerEntrySum: Record "Payable Vendor Ledger Entry" temporary; TempPayableVendorLedgerEntry: Record "Payable Vendor Ledger Entry" temporary)
    begin
        TempPayableVendorLedgerEntrySum.SetRange(Priority, TempPayableVendorLedgerEntry.Priority);
        TempPayableVendorLedgerEntrySum.SetRange("Vendor No.", TempPayableVendorLedgerEntry."Vendor No.");
        TempPayableVendorLedgerEntrySum.SetRange("Currency Code", TempPayableVendorLedgerEntry."Currency Code");
        TempPayableVendorLedgerEntrySum.SetRange(Positive, TempPayableVendorLedgerEntry.Positive);
        TempPayableVendorLedgerEntrySum.SetRange(Future, TempPayableVendorLedgerEntry.Future);
    end;
