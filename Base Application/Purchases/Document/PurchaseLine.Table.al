                    ...
                    "VAT Calculation Type"::"No Taxable VAT":
                        begin
                            Amount :=
                              Round(
                                "Amount Including VAT" /
                                (1 + (1 - GetVatBaseDiscountPct(PurchHeader) / 100) * ("VAT %" + "EC %") / 100),
                                Currency."Amount Rounding Precision");
                            "VAT Base Amount" :=
                              Round(Amount * (1 - GetVatBaseDiscountPct(PurchHeader) / 100), Currency."Amount Rounding Precision");
...
#pragma warning restore AA0074
                    ...
                    "VAT Calculation Type"::"No Taxable VAT":
                        begin
                            ...
                            "Amount Including VAT" :=
                              ...
                              Round(
                                (TotalAmount + Amount) * (GetVatBaseDiscountPct(PurchHeader) / 100) * ("VAT %" + "EC %") / 100,
                    ...
                    "VAT Calculation Type"::"No Taxable VAT":
                        begin
                            ...
                            "Amount Including VAT" :=
                              TotalAmount + Amount +
                              Round(
                                (TotalAmount + Amount) * (1 - GetVatBaseDiscountPct(PurchHeader) / 100) * ("VAT %" + "EC %") / 100,
                                Currency."Amount Rounding Precision", Currency.VATRoundingDirection()) -
                    ...
                    if VATAmountLine.Modified then begin
                        ...
                        if QtyType = QtyType::General then begin
                            if PurchHeader."Prices Including VAT" then begin
                                if (VATAmountLine.CalcLineAmount() = 0) or (PurchLine."Line Amount" = 0) then begin
                                    ...
                                    VATAmount :=
                                      ...
                                      VATAmountLine."EC Amount" * PurchLine.CalcLineAmount() / VATAmountLine.CalcLineAmount();
                                    NonDedVATAmount :=
                                ...
                                if PurchLine."VAT Calculation Type" = PurchLine."VAT Calculation Type"::"Full VAT" then begin
                                    VATAmount := PurchLine.CalcLineAmount() - PurchLine."Pmt. Discount Amount";
                                    NewAmount := 0;
                                    NewVATBaseAmount := 0;
                                end else begin
                            ...
                            if LineAmountToInvoice = 0 then begin
                                ...
                                PurchLine."EC Difference" := 0;
                                NonDedVATAmount := 0;
                                NonDeductibleVAT.InitNonDeductibleVATDiff(Rec);
                            end else begin
                                PurchLine."VAT Difference" := Round(VATDifference, Currency."Amount Rounding Precision");
                                PurchLine."EC Difference" := Round(ECDifference, Currency."Amount Rounding Precision");
                                NonDedVATAmount :=
                        ...
                        TempVATAmountLineRemainder."EC Difference" := ECDifference - PurchLine."EC Difference";
                    ...
                    then begin
                        ...
                        then
                            VATAmountLine.InsertNewLine(
                              PurchLine."VAT Identifier", PurchLine."VAT Calculation Type", PurchLine."Tax Group Code", PurchLine."Use Tax", VATPostingSetup."VAT %",
                              PurchLine."Line Amount" >= 0, false, VATPostingSetup."EC %", PurchLine."Non-Deductible VAT %")
                        else
                            VATAmountLine.InsertNewLine(
                              PurchLine."VAT Identifier", PurchLine."VAT Calculation Type", PurchLine."Tax Group Code", PurchLine."Use Tax", 0, PurchLine."Line Amount" >= 0, false, 0, PurchLine."Non-Deductible VAT %");
    ...
    begin
        PurchaseLine.SetLoadFields(
            ...
            "Unit Cost", "Amount Including VAT", Amount, "Line Amount", "Inv. Discount Amount", "Inv. Disc. Amount to Invoice", "VAT Difference", "VAT Base Amount", "EC Difference", "Pmt. Discount Amount",
            ...
            "System-Created Entry", "VAT Identifier", "VAT Calculation Type", "Tax Group Code", "VAT %", "Allow Invoice Disc.", "Prepayment Line", "Completely Received", "EC %");
    end;
