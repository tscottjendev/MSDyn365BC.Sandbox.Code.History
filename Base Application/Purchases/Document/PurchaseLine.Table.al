...
#pragma warning restore AA0074
    ...
    begin
                    ...
                    "VAT Calculation Type"::"Reverse Charge VAT":
                        begin
                            ...
                            if "VAT %" <> 0 then
                                "VAT Base Amount" :=
                                  Round(
                                    (TotalAmount + Amount) * (1 - GetVatBaseDiscountPct(PurchHeader) / 100),
                                    Currency."Amount Rounding Precision") -
                                  TotalVATBaseAmount
                            else
                                "VAT Base Amount" := Amount;
        ...
        OnAfterUpdateVATAmounts(Rec, TotalLineAmount, TotalInvDiscAmount, TotalAmount, TotalAmountInclVAT, TotalVATDifference, TotalQuantityBase, TotalVATBaseAmount);
    end;
                    ...
                    if VATAmountLine.Modified then begin
                        ...
                        if QtyType = QtyType::General then begin
                            if PurchHeader."Prices Including VAT" then begin
                                ...
                                if (PurchLine."VAT %" <> 0) and (VATAmountLine.CalcLineAmount() <> 0) then
                                    NewVATBaseAmount :=
                                      TempVATAmountLineRemainder."VAT Base (Lowered)" +
                                      VATAmountLine."VAT Base" * (1 - GetVatBaseDiscountPct(PurchHeader) / 100) *
                                      PurchLine.CalcLineAmount() / VATAmountLine.CalcLineAmount()
                                else
                        ...
                        if QtyType = QtyType::General then begin
                            ...
                            if PurchLine."VAT %" <> 0 then
                                PurchLine."VAT Base Amount" := Round(NewVATBaseAmount, Currency."Amount Rounding Precision")
                            else
                        ...
                        TempVATAmountLineRemainder."VAT Base (Lowered)" := NewVATBaseAmount - PurchLine."VAT Base Amount";
        ...
        if PurchLine.FindSet() then
            repeat
                if not PurchLine.ZeroAmountLine(QtyType) then begin
                    ...
                    if VATPostingSetupLocal.Get(PurchLine."VAT Bus. Posting Group", PurchLine."VAT Prod. Posting Group") then;
                    case QtyType of
                        QtyType::General:
                            begin
                                ...
                                if VATPostingSetupLocal."VAT Calculation Type" = VATPostingSetupLocal."VAT Calculation Type"::"Reverse Charge VAT" then
                                    VATAmountLine."VAT Base (Lowered)" +=
                                        Round(PurchLine."VAT Base Amount" * (100 - PurchHeader."VAT Base Discount %") / 100, Currency."Amount Rounding Precision")
                                else
                                    VATAmountLine."VAT Base (Lowered)" += PurchLine."VAT Base Amount";
    ...
    [IntegrationEvent(false, false)]
    local procedure OnAfterUpdateVATAmounts(var PurchaseLine: Record "Purchase Line"; TotalLineAmount: Decimal; TotalInvDiscAmount: Decimal; TotalAmount: Decimal; TotalAmountInclVAT: Decimal; TotalVATDifference: Decimal; TotalQuantityBase: Decimal; TotalVATBaseAmount: Decimal)
    begin
    end;
