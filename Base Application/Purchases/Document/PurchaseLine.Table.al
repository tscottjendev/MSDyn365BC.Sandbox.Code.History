            ...
            begin
                ...
                then begin
                    ...
                    if PurchSetup."Use Vendor's Tax Area Code" then
                        if ("Buy-from Vendor No." <> Vend."No.") and ("Buy-from Vendor No." <> '') then
                            Vend.Get("Buy-from Vendor No.");
                    if not VendorLocation.ReadPermission then
                        VendorLocation."Business Presence" := true
                    else
...
#pragma warning restore AA0074
    ...
    begin
        ...
        UpdateVATAmountsForReleasedDocument();
        ...
        if ("Line Amount" = "Inv. Discount Amount") and not
            ...
            then begin
                ...
                if not PurchLine2.IsEmpty() then begin
                    PurchLine2.CalcSums(
                      "Line Amount", "Inv. Discount Amount", Amount, "Amount Including VAT", "Quantity (Base)", "VAT Difference", "Tax To Be Expensed");
    ...
    begin
        IsHandled := false;
        OnBeforeUpdateVATAmountsForReleasedDocument(Rec, IsHandled);
        if IsHandled then
            exit;
        ...
        if PurchHeader.Status = PurchHeader.Status::Released then
            if PurchHeader."Tax Area Code" = '' then
                ...
                if (CurrFieldNo <> 0) and (("Over-Receipt Quantity" = 0) and (xRec."Over-Receipt Quantity" = 0)) then
                    Error(Text1020002);
        ...
        if PurchLine.FindSet() then
            repeat
                if not PurchLine.ZeroAmountLine(QtyType) then begin
                    ...
                    VATAmountLine.Get(PurchLine."VAT Identifier", PurchLine."VAT Calculation Type", PurchLine."Tax Group Code", PurchLine."Tax Area Code", PurchLine."Use Tax", PurchLine."Line Amount" >= 0);
                    if VATAmountLine.Modified then begin
                        if not TempVATAmountLineRemainder.Get(
                             PurchLine."VAT Identifier", PurchLine."VAT Calculation Type", PurchLine."Tax Group Code", PurchLine."Tax Area Code", PurchLine."Use Tax", PurchLine."Line Amount" >= 0)
                        then begin
                ...
                if not PurchLine.ZeroAmountLine(QtyType) then begin
                    ...
                    then begin
                        VATAmountLine.InsertNewLine(
                          PurchLine."VAT Identifier", PurchLine."VAT Calculation Type", PurchLine."Tax Group Code", PurchLine."Tax Area Code", PurchLine."Use Tax", PurchLine."VAT %", PurchLine."Line Amount" >= 0, false, PurchLine."Non-Deductible VAT %");
    ...
    procedure CalcSalesTaxLines(var PurchHeader: Record "Purchase Header"; var PurchLine: Record "Purchase Line")
    var
        TaxArea: Record "Tax Area";
    begin
        if PurchHeader."Tax Area Code" = '' then
        ...
        if TaxArea."Use External Tax Engine" then
            ...
            PurchLine.SetFilter("Tax Group Code", '<>%1', '');
    ...
    procedure TestExpenseCapitalize()
    var
        TaxAreaLine: Record "Tax Area Line";
        TaxDetail: Record "Tax Detail";
        Text1020000: Label '%1 cannot be set because %2 record %3, %4 is set for Expense/Capitalize.';
    begin
        if not "Use Tax" then
            exit;
    ...
    begin
        ...
        if SalesTaxDifference.AnyTaxDifferenceRecords(
             Enum::"Sales Tax Document Area"::Purchase.AsInteger(), "Document Type".AsInteger(), "Document No.")
        then
    ...
    begin
        ...
        if "Tax Liable" and ("Tax Area Code" <> '') and ("VAT Calculation Type" = "VAT Calculation Type"::"Sales Tax") then begin
            ...
            DeferralAmount := TempPurchaseLine.Amount + TempPurchaseLine."Tax To Be Expensed";
    ...
    [IntegrationEvent(false, false)]
    local procedure OnBeforeCheckForTaxDifferences(var PurchaseLine: Record "Purchase Line"; var IsHandled: Boolean)
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnBeforeUpdateVATAmountsForReleasedDocument(var PurchaseLine: Record "Purchase Line"; var IsHandled: Boolean)
    begin
    end;
