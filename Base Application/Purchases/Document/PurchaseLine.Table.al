...
#pragma warning restore AA0074
    ...
    begin
        ...
        if VATPostingSetup.Get("VAT Bus. Posting Group", "VAT Prod. Posting Group") and
           VATPostingSetup."Calc. Prop. Deduction VAT"
        then begin
            VATAmount := "Amount Including VAT" - Amount;
            VATAmount -= Round(VATAmount * VATPostingSetup."Proportional Deduction VAT %" / 100);
        end;
        if "VAT Base Amount" <> 0 then
            DeferralAmount := "VAT Base Amount" + VATAmount
        else
