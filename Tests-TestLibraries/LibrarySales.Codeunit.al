codeunit 130509 "Library - Sales"
{
    ...
    begin
        ...
        Customer.Validate("Payment Terms Code", LibraryERM.FindPaymentTermsIT());  // Mandatory for posting in ES build
        ...
        LibraryERM.SetDefaultSalesOperationType(Customer."VAT Bus. Posting Group");
    ...
    [Scope('OnPrem')]
    procedure CreateCustomerBillHeader(var CustomerBillHeader: Record "Customer Bill Header"; BankAccountNo: Code[20]; PaymentMethodCode: Code[10]; Type: Enum "Customer Bill Type")
    begin
        CustomerBillHeader.Init();
        CustomerBillHeader.Insert(true);
        CustomerBillHeader.Validate("Bank Account No.", BankAccountNo);
        CustomerBillHeader.Validate("Payment Method Code", PaymentMethodCode);
        CustomerBillHeader.Validate(Type, Type);
    ...
    begin
        ...
        SalesHeader.Validate("Operation Type", LibraryERM.GetDefaultOperationType(SellToCustomerNo, DATABASE::Customer));
        if SalesHeader."Payment Terms Code" <> '' then
            SalesHeader.Validate("Due Date", CalculateDueDate(SalesHeader."Payment Terms Code", SalesHeader."Document Date"))
        else
    ...
    begin
        ...
        CombineReturnReceiptsReport.InitializeRequest(
    ...
    procedure FindCustomer(var Customer: Record Customer)
    begin
    ...
    begin
        ...
        if SalesHeader."Operation Type" = '' then
            SalesHeader.Validate("Operation Type", LibraryERM.GetDefaultOperationType(SalesHeader."Sell-to Customer No.", DATABASE::Customer));
        if (SalesHeader."Applies-to Doc. No." <> '') and (SalesHeader."Applies-to Occurrence No." = 0) then
            SalesHeader.Validate("Applies-to Occurrence No.", 1);
        if SalesHeader."Operation Occurred Date" = 0D then
            SalesHeader.Validate("Operation Occurred Date", SalesHeader."Posting Date");
        SalesHeader.Modify(true);
            ...
            SalesHeader."Document Type"::Invoice, SalesHeader."Document Type"::"Credit Memo":
                if SalesHeader.Invoice and (SalesHeader."Posting No. Series" <> '') then begin
                    if (SalesHeader."Posting No." = '') then
                        SalesHeader."Posting No." := NoSeries.GetNextNo(SalesHeader."Posting No. Series", LibraryUtility.GetNoSeriesLastUsedDate(SalesHeader."Posting No. Series"));
                ...
                begin
                    if SalesHeader.Ship and (SalesHeader."Shipping No. Series" <> '') then begin
                        if (SalesHeader."Shipping No." = '') then
                            SalesHeader."Shipping No." := NoSeries.GetNextNo(SalesHeader."Shipping No. Series", LibraryUtility.GetNoSeriesLastUsedDate(SalesHeader."Shipping No. Series"));
                ...
                begin
                    if SalesHeader.Receive and (SalesHeader."Return Receipt No. Series" <> '') then begin
                        if (SalesHeader."Return Receipt No." = '') then
                            SalesHeader."Return Receipt No." := NoSeries.GetNextNo(SalesHeader."Return Receipt No. Series", LibraryUtility.GetNoSeriesLastUsedDate(SalesHeader."Return Receipt No. Series"));
    ...
    procedure SetFatturaPAElectronicFormat(NewValue: Code[20])
    begin
        SalesReceivablesSetup.Get();
        SalesReceivablesSetup.Validate("Fattura PA Electronic Format", NewValue);
        SalesReceivablesSetup.Modify();
    end;
    ...
    begin
        SalesReceivablesSetup.Get();
        SalesReceivablesSetup.Validate("Validate Document On Posting", NewValue);
        SalesReceivablesSetup.Modify();
    end;
    ...
    procedure CalculateDueDate("Code": Code[20]; DocumentDate: Date): Date
    var
